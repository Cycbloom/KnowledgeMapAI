...
const requireAuthLocal = async (req, _res, next) => {
...
const requireAuthSupabase = async (req, _res, next) => {
    const authHeader = req.headers.authorization;
    if (!authHeader) {
        throw new AppError('Authorization header missing', 401, ErrorCodes.AUTH_HEADER_MISSING);
    }
...
const optionalAuthLocal = async (req, _res, next) => {
    const token = extractToken(req);
    if (!token) {
        return next();
    }
...
const optionalAuthSupabase = async (req, _res, next) => {
    const authHeader = req.headers.authorization;
    if (!authHeader) {
...
const requireAdminLocal = async (req, res, next) => {
...
const requireAdminSupabase = async (req, res, next) => {
