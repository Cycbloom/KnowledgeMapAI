...
import { chatSchema, tutorChatSchema, extractConceptsSchema, suggestNextTopicSchema } from '../../schemas/index.js';
...
router.post('/chat', requireAuth, validate(chatSchema), async (req, res) => {
    ...
    setSSEHeaders(res);
    if (!provider.hasKey) {
        const mockContent = getMockResponse('chat', message);
    ...
    try {
        ...
        if (context_node_ids && context_node_ids.length > 0) {
            ...
            const relatedEdges = edges.filter((e) => context_node_ids.includes(e.source_knowledge_point_id) && context_node_ids.includes(e.target_knowledge_point_id));
        ...
        const systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, 'chat', { contextText }, req.user.id, graph_id);
        const messages = [
...
router.post('/tutor-chat', requireAuth, validate(tutorChatSchema), async (req, res) => {
    const { message, graph_id, history = [], context_node_ids, mode = 'free', provider: providerType, model } = req.body;
    const provider = providerType ? await getAIProvider(providerType) : await getAIProviderForTask('text');
    setSSEHeaders(res);
    if (!provider.hasKey) {
        const mockContent = await aiService.tutorChat([{ role: 'user', content: message }], { mode }, { provider: providerType, model });
        const chunks = mockContent.split('');
        const sendMockChunks = async () => {
            for (const chunk of chunks) {
                sendStreamChunk(res, chunk);
...
router.post('/extract-concepts', requireAuth, validate(extractConceptsSchema), async (req, res) => {
...
router.post('/suggest-next-topic', requireAuth, validate(suggestNextTopicSchema), async (req, res) => {
