...
import { ttsSchema, ttsVoicesSchema } from '../../schemas/index.js';
import { ErrorCodes } from '../../constants/errorCodes.js';
import { AppError } from '../../middleware/errorHandler.js';
import { getAIProviderForTask } from '../../services/ai/factory.js';
import { logger } from '../../utils/logger.js';
const router = Router();
router.get('/tts/voices', requireAuth, validate(ttsVoicesSchema), async (_req, res) => {
    try {
        const voices = [
...
router.post('/tts', requireAuth, validate(ttsSchema), async (req, res) => {
...
router.get('/tts/health', requireAuth, async (_req, res) => {
    try {
        const provider = await getAIProviderForTask('tts');
        if (provider.hasKey) {
            res.json({
