...
router.put('/:id', requireAuth, async (req, res) => {
    ...
    if (!existing)
        throw new AppError('Action not found', 404, ErrorCodes.RESOURCE_NOT_FOUND);
...
router.post('/execute', requireAuth, async (req, res) => {
    ...
    const result = await aiActionService.executeAction(action_id, node_id, userId, graph_id || 'none');
    res.json(result);
});
export default router;
//# sourceMappingURL=aiActions.js.map