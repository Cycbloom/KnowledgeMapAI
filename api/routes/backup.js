...
import { createBackup, readBackupFile, backupService, } from "../services/common/backupService.js";
...
router.post("/restore/:id", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (existingGraphs.data && existingGraphs.data.length > 0) {
            const graphIds = existingGraphs.data.map((g) => g.id);
...
async function restoreBackupData(supabase, userId, data) {
    const stats = {
        graphs: 0,
        nodes: 0,
        edges: 0,
    ...
    const oldToNewGraphIds = new Map();
    const oldToNewKnowledgePointIds = new Map();
    if (data.graphs && data.graphs.length > 0) {
        ...
        const { data: insertedGraphs, error: graphsError } = await supabase
            ...
            .select();
        if (graphsError)
            throw new Error(`导入图谱失败: ${graphsError.message}`);
