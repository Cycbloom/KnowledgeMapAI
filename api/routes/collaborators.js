...
const router = Router();
router.get("/:invitationToken/info", optionalAuth, async (req, res) => {
    try {
...
router.post("/graphs/:graphId/share", requireAuth, async (req, res) => {
    try {
...
router.post("/:invitationToken/join", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: "未授权" });
