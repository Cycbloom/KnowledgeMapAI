...
const router = Router();
// Export graph data
router.all('/export/:format', requireAuth, async (req, res) => {
    const { format } = req.params;
    const { graph_id } = req.query;
    // Handle POST body for advanced PDF options
    const { options } = req.body;
    if (!graph_id)
        return res.status(400).json({ error: '必须提供 graph_id' });
    ...
    const nodes = (graphNodesResult.data || []).map((gn) => {
    ...
    if (format === 'json') {
        ...
        const incomingEdges = new Set(); // target_knowledge_point_ids
        edges?.forEach((e) => {
        ...
        const getHeaderPrefix = (level, depth) => {
            switch (level) {
                case 'root': return '## ';
                case 'core': return '### ';
                case 'sub': return '#### ';
        ...
        const renderNode = (node, depth) => {
            ...
            if (node.content) {
                ...
                if (content) {
                    ...
                    md += `${content.split('\n').map((line) => `${contentPrefix}${line}`).join('\n')}\n\n`;
                }
                else {
                    md += '\n';
                }
        ...
        // Use PDF Service
        try {
            pdfService.generateReport(graph, nodes || [], edges || [], options || {}, // Pass options from body (screenshot, etc.)
            res);
        }
        catch (e) {
...
// Import Markdown
router.post('/import/markdown', requireAuth, async (req, res) => {
...
router.post('/import', requireAuth, validate(importDataSchema), async (req, res) => {
