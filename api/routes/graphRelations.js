...
import { graphRelationService } from "../services/graph/index.js";
...
router.get("/:graphId/relations", requireAuth, async (req, res) => {
    ...
    try {
        ...
        relations.outgoing.forEach((r) => {
            ...
            if (r.relation_type === "prerequisite")
                prerequisites.push(relation);
            else if (r.relation_type === "extension")
                extensions.push(relation);
            else
...
router.post("/:graphId/prerequisite-graph", requireAuth, validate(createPrerequisiteSchema), async (req, res) => {
    const { graphId } = req.params;
    ...
    try {
        ...
        if (duplicateCheck.isDuplicate && duplicateCheck.similarGraphs[0]) {
            ...
            isNew = true;
            if (auto_generate) {
                await taskService.createTask(req.user.id, "recursive_graph_generation", {
...
router.post("/:graphId/prerequisite-graphs/batch", requireAuth, validate(batchCreateSchema), async (req, res) => {
    const { graphId } = req.params;
        ...
        for (const item of topics) {
            ...
            if (existingGraph) {
                ...
                if (newGraph) {
                    ...
                    const task = await taskService.createTask(req.user.id, "recursive_graph_generation", {
...
router.delete("/:graphId/relations/:relationId", requireAuth, async (req, res) => {
...
router.post("/relations", requireAuth, validate(createRelationSchema), async (req, res) => {
...
router.delete("/relations/:relationId", requireAuth, async (req, res) => {
    const { relationId } = req.params;
    const supabase = req.supabase;
    try {
...
router.post("/:graphId/infinite-expand", requireAuth, validate(infiniteExpansionSchema), async (req, res) => {
    const { graphId } = req.params;
...
//# sourceMappingURL=graphRelations.js.map