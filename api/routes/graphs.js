...
router.get("/map/analyze", requireAuth, async (req, res) => {
    const supabase = req.supabase;
    const userId = req.user.id;
    const { data: graphs } = await supabase
        .from("knowledge_graphs")
    ...
    const { data: relations } = await supabase
        ...
        .or(`source_graph_id.in.(${graphIds.join(",")}),target_graph_id.in.(${graphIds.join(",")})`);
    const connectedGraphIds = new Set();
    (relations || []).forEach((r) => {
    ...
    const graphRelationCount = new Map();
    (relations || []).forEach((r) => {
...
router.post("/check-topic", requireAuth, validate({ body: checkTopicSchema }), async (req, res) => {
    const { topic, exclude_graph_id } = req.body;
    const result = await graphService.checkTopicDuplicate(req.supabase, req.user.id, topic, exclude_graph_id);
    res.json({
...
router.post("/", requireAuth, validate({ body: createGraphSchema }), async (req, res) => {
...
router.get("/:id", optionalAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
...
router.put("/:id", requireAuth, validate({ params: uuidParamsSchema, body: updateGraphSchema }), async (req, res) => {
    const { id } = req.params;
...
router.put("/:id/share", requireAuth, validate({ params: uuidParamsSchema, body: shareGraphSchema }), async (req, res) => {
    const { id } = req.params;
...
router.put("/:id/favorite", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
...
router.post("/batch/restore", requireAuth, validate({ body: batchOperationSchema }), async (req, res) => {
    const { ids } = req.body;
...
router.delete("/batch/permanent", requireAuth, validate({ body: batchOperationSchema }), async (req, res) => {
    const { ids } = req.body;
...
// Restore a graph
router.post("/:id/restore", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    await graphService.restoreGraph(req.supabase, id, req.user.id);
    res.json({ message: "图谱已恢复" });
});
// Permanently Delete a graph
router.delete("/:id/permanent", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
...
router.get("/:id/nodes", optionalAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id || null;
...
router.get("/:id/node-status", optionalAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id || null;
    const data = userId
...
router.get("/:id/learning-path", optionalAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id || null;
...
router.get("/:id/analyze", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    try {
...
router.get("/:id/missing-connections", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    const maxSuggestions = parseInt(req.query.max) || 10;
    try {
