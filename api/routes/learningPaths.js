...
router.get("/", requireAuth, validate({ query: listQuerySchema }), async (req, res) => {
    const { status } = req.query;
    const data = await learningPathService.getLearningPaths(req.supabase, req.user.id, status);
    res.json(data);
});
router.post("/", requireAuth, validate({ body: createPathSchema }), async (req, res) => {
    const data = await learningPathService.createLearningPath(req.supabase, req.user.id, req.body);
    res.status(201).json(data);
});
router.get("/:id", requireAuth, validate({ params: uuidParamSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await learningPathService.getLearningPath(req.supabase, id, req.user.id);
    if (!data) {
        throw new AppError("学习路径不存在", 404, ErrorCodes.NOT_FOUND);
    }
    res.json(data);
});
router.put("/:id", requireAuth, validate({ params: uuidParamSchema, body: updatePathSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await learningPathService.updateLearningPath(req.supabase, id, req.user.id, req.body);
    res.json(data);
});
router.delete("/:id", requireAuth, validate({ params: uuidParamSchema }), async (req, res) => {
    const { id } = req.params;
    const hardDelete = req.query.hard === "true";
    await learningPathService.deleteLearningPath(req.supabase, id, req.user.id, hardDelete);
    res.json({
        message: hardDelete ? "学习路径已永久删除" : "学习路径已归档",
    });
});
router.post("/:id/nodes", requireAuth, validate({ params: uuidParamSchema, body: addNodeSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await learningPathService.addNodeToPath(req.supabase, id, req.user.id, req.body);
    res.status(201).json(data);
});
router.put("/:id/nodes/:nodeId/status", requireAuth, validate({ params: nodeIdParamSchema, body: updateNodeStatusSchema }), async (req, res) => {
    const { id, nodeId } = req.params;
    const data = await learningPathService.updateNodeStatus(req.supabase, id, nodeId, req.user.id, req.body);
    res.json(data);
});
router.put("/:id/nodes/reorder", requireAuth, validate({ params: uuidParamSchema, body: reorderNodesSchema }), async (req, res) => {
    const { id } = req.params;
    const { nodeOrders } = req.body;
    await learningPathService.reorderNodes(req.supabase, id, req.user.id, nodeOrders);
    res.json({ message: "节点顺序已更新" });
});
router.delete("/:id/nodes/:nodeId", requireAuth, validate({ params: nodeIdParamSchema }), async (req, res) => {
    const { id, nodeId } = req.params;
    await learningPathService.removeNodeFromPath(req.supabase, id, nodeId, req.user.id);
    res.json({ message: "节点已移除" });
});
router.get("/:id/progress", requireAuth, validate({ params: uuidParamSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await learningPathService.getPathProgress(req.supabase, id, req.user.id);
    res.json(data);
});
router.put("/:id/progress", requireAuth, validate({ params: uuidParamSchema, body: updateProgressSchema }), async (req, res) => {
    const { id } = req.params;
    const { node_id, ...input } = req.body;
    const data = await learningPathService.updateProgress(req.supabase, id, node_id, req.user.id, input);
    res.json(data);
});
router.post("/:id/plans", requireAuth, validate({ params: uuidParamSchema, body: createPlanSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await learningPathService.createDailyPlan(req.supabase, id, req.user.id, req.body);
    res.status(201).json(data);
});
router.get("/:id/plans", requireAuth, validate({ params: uuidParamSchema, query: plansQuerySchema }), async (req, res) => {
    const { id } = req.params;
    const { start_date, end_date } = req.query;
    const data = await learningPathService.getDailyPlans(req.supabase, id, req.user.id, start_date, end_date);
    res.json(data);
});
router.get("/:id/plans/:date", requireAuth, validate({ params: dateParamSchema }), async (req, res) => {
    const { id, date } = req.params;
    const data = await learningPathService.getDailyPlan(req.supabase, id, req.user.id, date);
    if (!data) {
        throw new AppError("未找到该日期的计划", 404, ErrorCodes.NOT_FOUND);
    }
    res.json(data);
});
router.put("/:id/plans/:date", requireAuth, validate({ params: dateParamSchema, body: updatePlanSchema }), async (req, res) => {
    const { id, date } = req.params;
    const existingPlan = await learningPathService.getDailyPlan(req.supabase, id, req.user.id, date);
    if (!existingPlan) {
        throw new AppError("未找到该日期的计划", 404, ErrorCodes.NOT_FOUND);
    }
    const data = await learningPathService.updatePlanStatus(req.supabase, existingPlan.id, req.user.id, req.body);
    res.json(data);
});
router.post("/generate", requireAuth, validate({ body: generatePathSchema }), async (req, res) => {
    const { graph_id, target_goal, target_knowledge_point_id, learning_style, daily_time_minutes, current_knowledge, provider: providerType, model, save_path, path_title, } = req.body;
    ...
    try {
        ...
        if (target_goal) {
            ...
            const ruleResult = generateRulePath(nodes, edges, progressMap, parentMap, childMap, target_knowledge_point_id, daily_time_minutes);
        ...
        const weeklyProgress = calculateWeeklyProgress(daily_time_minutes, totalEstimatedTime);
        const learningPath = {
            graphId: graph_id,
        ...
        if (save_path) {
            const savedPath = await learningPathService.createLearningPath(supabase, req.user.id, {
...
async function generateAIPath(supabase, userId, graphId, nodes, edges, progressMap, parentMap, childMap, targetGoal, learningStyle, dailyTimeMinutes, currentKnowledge, graphTitle, _providerType, model) {
    ...
    if (!provider.hasKey) {
        return generateRulePath(nodes, edges, progressMap, parentMap, childMap, undefined, dailyTimeMinutes);
    }
...
function generateRulePath(nodes, _edges, progressMap, parentMap, childMap, targetNodeId, _dailyTimeMinutes) {
    ...
    for (const nodeId of sortedNodes) {
        ...
        const estimatedTime = calculateEstimatedTime(progress?.masteryLevel || 0, node.level, parents.length, children.length);
        stages.push({
            nodeId,
...
function topologicalSort(nodes, parentMap) {
    const sortedNodes = [];
...
function calculateEstimatedTime(masteryLevel, level = "normal", parentCount = 0, childCount = 0) {
    let baseTime = 15;
    switch (level) {
        case "beginner":
            baseTime = 25;
...
function generateSuggestions(stages, today) {
    const suggestions = [];
    ...
    const advancedNodes = stages.filter((s) => s.level === "advanced" && !s.isCompleted);
...
async function buildProgressMap(supabase, userId, nodes) {
...
function buildTodayPlan(stages, dailyTimeMinutes) {
    const todayPlan = [];
...
function calculateWeeklyProgress(dailyTimeMinutes, totalEstimatedTime) {
...
router.post("/:id/auto-schedule", requireAuth, validate({ body: autoScheduleSchema }), async (req, res) => {
    const { id } = req.params;
    const { start_date, daily_minutes } = req.body;
    const supabase = req.supabase;
    try {
        const result = await learningPathService.autoSchedulePath(supabase, id, req.user.id, {
            start_date,
            daily_minutes,
        });
        res.json({
        ...
        if (error instanceof AppError)
            throw error;
        throw new AppError(error.message || "自动排程失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
