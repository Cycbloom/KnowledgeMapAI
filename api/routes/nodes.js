...
router.post("/nodes", requireAuth, validate(createNodeSchema), async (req, res) => {
    const { graph_id, title, content, x_position, y_position, properties, level, is_accepted, learning_material, knowledge_point_id: existingKpId, reuse_existing = true, } = req.body;
    ...
    if (!knowledgePointId && reuse_existing) {
        try {
            if (title) {
                ...
                if (embedding) {
                    const similarKps = await knowledgePointService.searchSimilar(req.supabase, embedding, req.user.id, REUSE_SIMILARITY_THRESHOLD, 1);
...
router.get("/nodes/:id", requireAuth, async (req, res) => {
    const { id } = req.params;
    const { data: graphNode, error } = await req
...
router.put("/nodes/:id", requireAuth, validate(updateNodeSchema), async (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    const { data: existingNode, error: findError } = await req
        .supabase.from("graph_nodes")
        .select(`
      id,
      graph_id,
    ...
    if (updates.title !== undefined)
        kpUpdates.title = updates.title;
    if (updates.content !== undefined)
        kpUpdates.content = updates.content;
    ...
    const { data: updatedNode, error: refetchError } = await req
        .supabase.from("graph_nodes")
        .select(`
      id,
      graph_id,
...
router.get("/nodes/:id/related", requireAuth, async (req, res) => {
    const { id } = req.params;
    const limit = parseInt(req.query.limit) || 5;
    try {
        const { data: graphNode, error: nodeError } = await req
            .supabase.from("graph_nodes")
            .select(`
        id,
        knowledge_point_id,
      ...
      `)
        ...
        const kp = graphNode.knowledge_points;
        ...
        const relatedKps = await knowledgePointService.searchSimilar(req.supabase, embedding, req.user.id, 0.5, limit + 1);
...
router.delete("/nodes/:id", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const hardDeleteParam = req.query.hard_delete;
    const hardDelete = hardDeleteParam === "true" || hardDeleteParam === "1";
    const { data: graphNode, error: findError } = await req
        .supabase.from("graph_nodes")
        .select(`
      id,
      graph_id,
...
router.post("/nodes/batch-delete", requireAuth, validate(batchDeleteNodesSchema), async (req, res) => {
...
router.post("/nodes/batch-update-positions", requireAuth, validate(batchUpdatePositionsSchema), async (req, res) => {
...
router.get("/nodes/:id/knowledge-point-graphs", requireAuth, async (req, res) => {
    const { id } = req.params;
    const data = await knowledgePointService.getGraphs(req.supabase, id, req.user.id);
    res.json(data || []);
});
router.post("/edges", requireAuth, validate(createEdgeSchema), async (req, res) => {
...
router.delete("/edges/:id", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const { id } = req.params;
    const { data: edge, error } = await req
        ...
        .single();
    if (error)
        throw new AppError(error.message || "删除边失败", 500, ErrorCodes.INTERNAL_ERROR);
    if (!edge) {
