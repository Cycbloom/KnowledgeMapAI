...
router.post('/', requireAuth, validate({ body: createNotificationSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res.status(500).json({ error: 'Database connection not available' });
    }
...
router.put('/settings', requireAuth, validate({ body: updateSettingsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res.status(500).json({ error: 'Database connection not available' });
    }
