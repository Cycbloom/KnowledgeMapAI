...
import { createQuizSetSchema, updateQuizSetSchema, generateQuizSchema, regenerateCardSchema, uuidParamsSchema, } from "../schemas/index.js";
...
const router = Router();
router.get("/quiz-sets", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const err = error;
        logger.error("Error fetching quiz sets:", error);
        throw new AppError(err.message || "获取测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.get("/quiz-sets/:id", requireAuth, validate(uuidParamsSchema), async (req, res) => {
    const { id } = req.params;
    try {
        ...
        if (quizSetError || !quizSet) {
            throw new AppError("未找到测验集合", 404, ErrorCodes.RESOURCE_NOT_FOUND);
        }
...
router.post("/quiz-sets", requireAuth, validate(createQuizSetSchema), async (req, res) => {
    ...
    try {
        ...
        const err = error;
        logger.error("Error creating quiz set:", error);
        throw new AppError(err.message || "创建测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.put("/quiz-sets/:id", requireAuth, validate(updateQuizSetSchema), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const err = error;
        logger.error("Error updating quiz set:", error);
        throw new AppError(err.message || "更新测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.delete("/quiz-sets/:id", requireAuth, validate(uuidParamsSchema), async (req, res) => {
    const { id } = req.params;
    try {
        const { data: existing, error: fetchError } = await req
            .supabase.from("quiz_sets")
        ...
        if (deleteCardsError) {
            ...
            throw new AppError("删除测验卡片关联失败", 500, ErrorCodes.INTERNAL_ERROR);
        }
        ...
        const err = error;
        logger.error("Error deleting quiz set:", error);
        throw new AppError(err.message || "删除测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.post("/quiz-sets/generate", requireAuth, validate(generateQuizSchema), async (req, res) => {
    ...
    try {
        ...
        throw new AppError(err.message || "生成测验失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.post("/quiz-sets/:id/regenerate/:cardId", requireAuth, validate(regenerateCardSchema), async (req, res) => {
    ...
    try {
        ...
        if (cardLinkError || !cardLink) {
            throw new AppError("未找到测验卡片", 404, ErrorCodes.RESOURCE_NOT_FOUND);
        }
        const oldCard = cardLink.card;
        const displayOrder = cardLink.display_order;
        if (!oldCard?.knowledge_point_id) {
            throw new AppError("卡片缺少知识点关联", 400, ErrorCodes.VALIDATION_ERROR);
        }
        const { data: graphNode } = await req
            ...
            .single();
        if (!graphNode) {
            throw new AppError("未找到关联的知识点", 404, ErrorCodes.NODE_NOT_FOUND);
        }
        const config = quizSet.config || {};
        const types = config.types || [oldCard.card_type || "qa"];
        const difficulty = config.difficulty || "medium";
        const count = 1;
        const aiResult = await aiService.generateCards(graphNode.title || "", graphNode.content || "", {
            count,
            types,
            difficulty,
        ...
        const newCard = await studyService.createCard(req.supabase, {
            ...
            cardType: newCardData.type || newCardData.card_type || oldCard.card_type,
        ...
        throw new AppError(err.message || "重新生成卡片失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.post("/quiz-sets/:id/cards", requireAuth, validate(uuidParamsSchema), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const { data: existingLink } = await req
            ...
            .single();
        if (existingLink) {
            throw new AppError("该卡片已在测验集合中", 400, ErrorCodes.VALIDATION_ERROR);
        }
        ...
        if (insertError) {
            ...
            throw new AppError("添加卡片到测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
        }
        await req
            .supabase.from("quiz_sets")
            .update({
        ...
        throw new AppError(err.message || "添加卡片到测验集合失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
router.delete("/quiz-sets/:id/cards/:cardId", requireAuth, validate(uuidParamsSchema), async (req, res) => {
    const { id, cardId } = req.params;
    try {
        const { data: quizSet, error: quizSetError } = await req
            .supabase.from("quiz_sets")
        ...
        if (deleteError) {
            ...
            throw new AppError("从测验集合移除卡片失败", 500, ErrorCodes.INTERNAL_ERROR);
        }
        await req
            .supabase.from("quiz_sets")
            .update({
        ...
        throw new AppError(err.message || "从测验集合移除卡片失败", 500, ErrorCodes.INTERNAL_ERROR);
    }
});
export default router;
//# sourceMappingURL=quizSets.js.map