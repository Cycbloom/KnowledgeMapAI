...
router.get("/stats", requireAuth, validate({ query: getStatsQuerySchema }), async (req, res) => {
    ...
    const totalDuration = executions?.reduce((sum, e) => sum + (e.duration ?? 0), 0) ?? 0;
    const queueStats = {
...
router.get("/heatmap", requireAuth, validate({ query: getHeatmapQuerySchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.get("/analytics", requireAuth, async (req, res) => {
    ...
    try {
        const analytics = await taskAnalyticsService.getAnalytics(supabase, req.user.id);
...
router.post("/analytics/insights", requireAuth, async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    try {
        const insights = await taskAnalyticsService.generateInsights(supabase, req.user.id);
