...
import { z } from "zod";
import { createTaskDependencySchema, taskDependencyParamsSchema, } from "../../schemas/index.js";
const router = Router();
const uuidParamsSchema = z.object({
    id: z.string().uuid("无效的任务ID"),
});
async function checkCircularDependency(supabase, taskId, dependsOnTaskId, userId) {
...
router.post("/tasks/:id/dependencies", requireAuth, validate({ params: uuidParamsSchema, body: createTaskDependencySchema }), async (req, res) => {
    ...
    const hasCircular = await checkCircularDependency(supabase, id, depends_on_task_id, req.user.id);
    if (hasCircular) {
...
router.delete("/tasks/:id/dependencies/:dependencyId", requireAuth, validate({ params: taskDependencyParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.get("/tasks/:id/dependencies", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    const { data: dependencies, error } = await supabase
        .from("task_dependencies")
        .select("id, dependency_type, created_at, depends_on_task_id, scheduled_tasks!task_dependencies_depends_on_task_id_fkey(id, title, description, status, queue_level, priority)")
        .eq("task_id", id)
        .eq("user_id", req.user.id);
    if (error) {
...
router.get("/tasks/:id/dependents", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    const { data: dependents, error } = await supabase
        .from("task_dependencies")
        .select("id, dependency_type, created_at, task_id, scheduled_tasks!task_dependencies_task_id_fkey(id, title, description, status, queue_level, priority)")
