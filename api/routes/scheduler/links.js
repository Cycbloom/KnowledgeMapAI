...
router.post("/tasks/:id/links", requireAuth, validate(createLinkSchema), async (req, res) => {
...
router.get("/tasks/:id/links", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.put("/tasks/:id/links/:linkId", requireAuth, validate(updateLinkSchema), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.delete("/tasks/:id/links/:linkId", requireAuth, validate({ params: linkParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
