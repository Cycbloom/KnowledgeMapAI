...
import { z } from "zod";
import { createProgressPlanSchema, updateProgressSchema, } from "../../schemas/index.js";
...
function generateAverageAllocations(days) {
...
function generateDecreasingAllocations(days) {
    const allocations = [];
    ...
    if (Math.abs(total - 100) > 0.01 && allocations.length > 0) {
        allocations[allocations.length - 1].percentage =
            Math.round((allocations[allocations.length - 1].percentage + (100 - total)) * 100) / 100;
    }
    return allocations;
}
function generateIncreasingAllocations(days) {
    const decreasing = generateDecreasingAllocations(days);
    return decreasing.reverse();
}
function generateProgressAllocations(startDate, endDate, mode, customAllocations) {
    if (mode === "custom" && customAllocations && customAllocations.length > 0) {
        ...
        return customAllocations.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    }
...
router.post("/tasks/:id/progress-plan", requireAuth, validate({ params: uuidParamsSchema, body: createProgressPlanSchema }), async (req, res) => {
    ...
    const { id } = req.params;
    const { start_date, end_date, progress_mode, custom_allocations } = req.body;
    ...
    try {
        const allocations = generateProgressAllocations(new Date(start_date), new Date(end_date), progress_mode, custom_allocations);
...
router.put("/tasks/:id/progress-plan", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.get("/tasks/:id/progress-plan", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.post("/tasks/:id/progress", requireAuth, validate({ params: uuidParamsSchema, body: updateProgressSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    if (allPlans && allPlans.length > 0) {
        const totalActual = allPlans.reduce((sum, p) => sum + (p.actual_percentage || 0), 0);
