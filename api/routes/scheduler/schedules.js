...
import { z } from "zod";
import { createTaskScheduleSchema, updateTaskScheduleSchema, taskScheduleParamsSchema, } from "../../schemas/index.js";
import { logger } from "../../utils/logger.js";
const router = Router();
const uuidParamsSchema = z.object({
    id: z.string().uuid("无效的调度ID"),
});
function calculateNextRunAt(scheduleType, scheduleConfig) {
    const now = new Date();
    ...
    switch (scheduleType) {
        ...
        case "custom": {
            const intervalDays = scheduleConfig.interval_days || 1;
            if (nextRun <= now) {
        ...
        case "smart": {
            const baseInterval = scheduleConfig.base_interval || 3;
            if (nextRun <= now) {
...
router.post("/schedules", requireAuth, validate({ body: createTaskScheduleSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.put("/schedules/:id", requireAuth, validate({
    ...
    if (schedule_config !== undefined) {
        ...
        updateData.next_run_at = calculateNextRunAt(existingSchedule.schedule_type, schedule_config).toISOString();
    }
...
router.delete("/schedules/:id", requireAuth, validate({ params: taskScheduleParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.post("/schedules/:id/run", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    const nextRunAt = calculateNextRunAt(schedule.schedule_type, schedule.schedule_config);
    await supabase
        .from("task_schedules")
        .update({
