...
router.post("/tasks/:id/subtasks", requireAuth, validate(createSubtaskSchema), async (req, res) => {
    ...
    const { id } = req.params;
    const { title, description, priority, estimated_duration, due_date } = req.body;
...
router.get("/tasks/:id/subtasks", requireAuth, validate({ params: z.object({ id: z.string().uuid("无效的任务ID") }) }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.put("/tasks/:id/subtasks/:subtaskId", requireAuth, validate(updateSubtaskSchema), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    if (subtask.learning_path_node_id && updates.status === "completed") {
        try {
            ...
            logger.info(`Synced subtask ${subtaskId} completion with learning path node ${subtask.learning_path_node_id}`);
        }
        catch (syncError) {
...
router.delete("/tasks/:id/subtasks/:subtaskId", requireAuth, validate({ params: subtaskParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
