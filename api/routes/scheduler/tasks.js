...
import { z } from "zod";
import { createScheduledTaskSchema, updateScheduledTaskSchema, moveTaskSchema, reorderTasksSchema, } from "../../schemas/index.js";
const router = Router();
...
router.post("/tasks", requireAuth, validate(createScheduledTaskSchema), async (req, res) => {
    ...
    const { title, description, queue_level, estimated_duration, deadline, tags, knowledge_point_id, priority, task_type, total_duration, progress_mode, context, parent_task_id, } = req.body;
...
router.get("/tasks", requireAuth, validate({ query: getTasksQuerySchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    const { status, queue_level, limit, offset } = req.query;
    let query = supabase
        .from("scheduled_tasks")
        .select("*", { count: "exact" })
        .eq("user_id", req.user.id)
    ...
    if (tasks && tasks.length > 0) {
        ...
        for (const task of tasks) {
            ...
            task.has_subtasks = (stats?.total || 0) > 0;
        }
    }
    res.json({ success: true, data: tasks, total: count });
});
router.put("/tasks/reorder", requireAuth, validate({ body: reorderTasksSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.get("/tasks/:id", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.get("/tasks/:id/detail", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
    ...
    const { data: dependencies } = await supabase
        ...
        .select("id, task_id, depends_on_task_id, dependency_type, created_at, depends_on_task:scheduled_tasks!task_dependencies_depends_on_task_id_fkey(id, title, description, status, queue_level, priority)")
        .eq("task_id", id);
    const { data: dependents } = await supabase
        .from("task_dependencies")
        .select("id, task_id, depends_on_task_id, dependency_type, created_at, task:scheduled_tasks!task_dependencies_task_id_fkey(id, title, description, status, queue_level, priority)")
...
router.put("/tasks/:id", requireAuth, validate({ params: uuidParamsSchema, body: updateScheduledTaskSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.delete("/tasks/:id", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.post("/tasks/:id/start", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.post("/tasks/:id/pause", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.post("/tasks/:id/complete", requireAuth, validate({ params: uuidParamsSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
...
router.put("/tasks/:id/move", requireAuth, validate({ params: uuidParamsSchema, body: moveTaskSchema }), async (req, res) => {
    const supabase = req.supabase;
    if (!supabase) {
        return res
            .status(500)
