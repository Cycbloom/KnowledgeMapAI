...
router.get("/", optionalAuth, async (req, res) => {
    ...
    const data = await graphTemplateService.getTemplates(req.supabase, category);
    res.json(data);
});
...
router.post("/", requireAuth, validate({ body: createTemplateSchema }), async (req, res) => {
    const data = await graphTemplateService.createTemplate(req.supabase, req.user.id, req.body);
    res.status(201).json(data);
});
router.put("/:id", requireAuth, validate({ params: uuidParamsSchema, body: updateTemplateSchema }), async (req, res) => {
    const { id } = req.params;
    const data = await graphTemplateService.updateTemplate(req.supabase, id, req.user.id, req.body);
    res.json(data);
});
...
router.post("/from-template", requireAuth, validate({ body: createGraphFromTemplateSchema }), async (req, res) => {
    const { template_id, title, description } = req.body;
    const template = await graphTemplateService.getTemplate(req.supabase, template_id);
    if (!template) {
        throw new AppError("模板不存在", 404, ErrorCodes.RESOURCE_TEMPLATE_NOT_FOUND);
    }
    const graph = await graphService.createGraph(req.supabase, req.user.id, title || template.name, description || template.description);
    if (template.nodes && template.nodes.length > 0) {
        const { data: knowledgePoints, error: kpError } = await req
            .supabase.from("knowledge_points")
            .insert(template.nodes.map((node) => ({
            user_id: req.user.id,
            title: node.title,
            ...
            .select("id, title");
        if (kpError) {
            console.error("Failed to create knowledge points from template:", kpError);
        }
        else if (knowledgePoints && knowledgePoints.length > 0) {
            const nodeTitleToId = new Map(knowledgePoints.map((kp) => [kp.title, kp.id]));
            ...
            if (graphNodesData.length > 0) {
                ...
                if (gnError) {
                    console.error("Failed to create graph nodes from template:", gnError);
                }
                else if (insertedNodes &&
                    insertedNodes.length > 0 &&
                    template.edges &&
                    template.edges.length > 0) {
                    const kpIdToNodeId = new Map(insertedNodes.map((gn) => [gn.knowledge_point_id, gn.id]));
                    const edgesData = [];
                    ...
                    if (edgesData.length > 0) {
                        const { error: edgeError } = await req
                            ...
                            .insert(edgesData);
                        if (edgeError) {
                            console.error("Failed to create edges from template:", edgeError);
                        }
                    }
                }
            }
