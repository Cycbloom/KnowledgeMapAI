...
const createTemplateSchema = z.object({
  ...
  category: z.enum(['learning', 'story', 'project', 'analysis', 'custom']).optional(),
...
router.get('/', optionalAuth, async (req: AuthRequest, res: Response) => {
  ...
  const data = await graphTemplateService.getTemplates(req.supabase!, category as string);
  res.json(data);
});
...
router.post('/from-template', requireAuth, validate({ body: createGraphFromTemplateSchema }), async (req: AuthRequest, res: Response) => {
  ...
  if (template.nodes && template.nodes.length > 0) {
    const { data: knowledgePoints, error: kpError } = await req.supabase!
    ...
    if (kpError) {
      ...
      const nodeTitleToId = new Map(knowledgePoints.map((kp: any) => [kp.title, kp.id]));
      
      const graphNodesData = template.nodes.map((node: any) => {
      ...
      if (graphNodesData.length > 0) {
        const { data: insertedNodes, error: gnError } = await req.supabase!
          .from('graph_nodes')
        ...
        } else if (insertedNodes && insertedNodes.length > 0 && template.edges && template.edges.length > 0) {
          const kpIdToNodeId = new Map(insertedNodes.map((gn: any) => [gn.knowledge_point_id, gn.id]));
          
          const edgesData: any[] = [];
          for (const edge of template.edges) {
            const sourceNode = template.nodes.find((n: any) => n.id === edge.source);
            const targetNode = template.nodes.find((n: any) => n.id === edge.target);
