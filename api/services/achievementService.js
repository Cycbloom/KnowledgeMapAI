...
export class AchievementService {
     ...
     */
    async updateDailyTask(userId, type, amount = 1) {
        const today = new Date().toISOString().split('T')[0];
        // Get task
     ...
     */
    async updateDailyTaskProgress(userId, type, currentTotal) {
        const today = new Date().toISOString().split('T')[0];
        const { data: task } = await supabaseAdmin
            .from('daily_tasks')
            .select('*')
     ...
     */
    async updateMasteredStats(userId) {
        const { count, error } = await supabaseAdmin
            .from('study_cards')
            .select('*', { count: 'exact', head: true })
            .eq('user_id', userId)
     ...
     */
    async updateCreationStats(userId) {
