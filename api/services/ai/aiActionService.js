...
export class AIActionService {
    async listActions(supabase, _userId, _graphId) {
            ...
            if (action.variables.includeParent) {
                ...
                if (edges && edges.length > 0) {
                    ...
                    if (parentGraphNodes && parentGraphNodes.length > 0) {
                        context.parents = parentGraphNodes.map((pgn) => {
            ...
            if (action.variables.includeChildren) {
                ...
                if (edges && edges.length > 0) {
                    ...
                    if (childGraphNodes && childGraphNodes.length > 0) {
                        context.children = childGraphNodes.map((cgn) => {
                    ...
                    if (siblingEdges && siblingEdges.length > 0) {
                        ...
                        if (siblingIds.length > 0) {
                            ...
                            if (siblingGraphNodes && siblingGraphNodes.length > 0) {
                                context.siblings = siblingGraphNodes.map((sgn) => {
    ...
    async handleActionResponse(action, responseContent, nodeId, userId, graphId) {
        ...
        if (action.target_mode === 'update_node') {
            const kpUpdates = {};
            if (parsed.content)
                kpUpdates.content = parsed.content;
            if (parsed.title)
                kpUpdates.title = parsed.title;
        ...
        if (action.target_mode === 'spawn_children') {
            if (parsed.children && Array.isArray(parsed.children)) {
                ...
                for (const child of parsed.children) {
                    const result = await createKnowledgePointWithGraphNode(supabaseAdmin, userId, {
                        graph_id: graphId,
                        title: child.title,
...
//# sourceMappingURL=aiActionService.js.map