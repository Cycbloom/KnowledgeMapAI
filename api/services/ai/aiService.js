...
import { getMockResponse, getMockCards, getMockBranchSuggestions, getMockConcepts, getMockNextTopics, getMockImageGraph, } from "./mock.js";
import { withTimeoutAndRetry, TimeoutError, RetryError, DEFAULT_TIMEOUT, } from "../../utils/retry.js";
...
function generateRequestKey(operation, params) {
...
export class AIService {
    ...
    async generateEmbeddingsBatch(texts) {
        ...
        if (provider.createEmbedding) {
            ...
            for (let i = 0; i < texts.length; i += concurrencyLimit) {
                ...
                const batchResults = await Promise.all(batch.map((text) => provider.createEmbedding(text).catch(() => null)));
    ...
    async chat(messages, options = {}) {
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                    messages,
                    model: options.model || provider.model,
                }), {
                    ...
                    onRetry: (attempt, error) => {
                        logger.warn(`Chat request retry attempt ${attempt}: ${error.message}`);
                    },
                });
    ...
    async generatePodcastScript(context, language = "zh") {
...
**主持人**: 请配置好 API Key 后，我将为你带来精彩的深度解读！`;
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                    ...
                    maxRetries: 3,
                    onRetry: (attempt, error) => {
                        logger.warn(`Generate Podcast Script retry attempt ${attempt}: ${error.message}`);
                    },
                });
                return completion.choices[0].message.content || "";
            });
    ...
    async generateCards(topic, content, options = {}) {
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                const promptParts = await Promise.all(types.map(async (type) => {
                    ...
                    const rendered = await promptService.getRenderedPrompt(supabaseAdmin, code, { count: Math.ceil(count / types.length), difficulty }, options.userId, options.graphId);
                ...
                if (!systemPrompt.trim()) {
                    systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, "generate_cards", {
                        count,
                        allowedTypes: types.join(", "),
                        context: context ? `Parent/Context Info: ${context}` : "",
                        difficulty,
                    }, options.userId, options.graphId);
                }
                else {
...
Context: ${context || "None"}\n\n${systemPrompt}`;
                ...
                const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                    ...
                    maxRetries: 3,
                    onRetry: (attempt, error) => {
                        logger.warn(`Generate Cards retry attempt ${attempt}: ${error.message}`);
                    },
                });
                const result = completion.choices[0].message.content || "";
                const parsed = parseAIResponse(result, "Generate Cards");
    ...
    async expandKnowledge(nodeTitle, nodeContent, existingNodes, childNodes, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
            return getMockResponse("expand", nodeTitle);
        }
        ...
        try {
            ...
            const systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, "expand_knowledge", templateContext, options.userId, options.graphId);
            const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                messages: [
                    { role: "system", content: systemPrompt },
                    {
                ...
                maxRetries: 3,
                onRetry: (attempt, error) => {
                    logger.warn(`Expand Knowledge retry attempt ${attempt}: ${error.message}`);
                },
            });
            ...
            const parsed = parseAIResponse(content, "Expand Knowledge");
    ...
    async getBranchSuggestions(nodeTitle, nodeContent, existingNodes, childNodes, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                ...
                const systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, "branch_suggestions", templateContext, options.userId, options.graphId);
                ...
                const content = completion.choices[0].message.content || "";
                const parsed = parseAIResponse(content, "Branch Suggestions");
    ...
    async generateGraphFromImage(imageBase64, options = {}) {
...
4. Respond in Chinese.`,
            ...
            const content = completion.choices[0].message.content || "";
            return parseAIResponse(content, "Image to Graph");
        }
        catch (error) {
            const err = error;
    ...
    async generateLearningMaterial(topic, context, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
    ...
    async suggestNextTopic(nodeTitle, nodeContent, _existingNodes, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                ...
                const content = completion.choices[0].message.content || "";
                const parsed = parseAIResponse(content, "Suggest Next Topic");
                return { suggestions: parsed.suggestions || [] };
            });
        }
        catch (error) {
    ...
    async tutorChat(messages, context = {}, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
...
5. All mathematical formulas must be wrapped in LaTeX: $inline$ or $$block$$`,
                ...
                maxRetries: 3,
                onRetry: (attempt, error) => {
                    logger.warn(`Tutor Chat retry attempt ${attempt}: ${error.message}`);
                },
            });
            return completion.choices[0].message.content || "";
        }
    ...
    async extractConcepts(text, existingNodes, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
...
Please respond in Chinese.`,
                    ...
                    maxRetries: 3,
                    onRetry: (attempt, error) => {
                        logger.warn(`Extract Concepts retry attempt ${attempt}: ${error.message}`);
                    },
                });
                const content = completion.choices[0].message.content || "";
                const parsed = parseAIResponse(content, "Extract Concepts");
    ...
    async analyzeCrossGraphConnections(graph1, graph2, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
        ...
        try {
            const graph1NodesText = graph1.nodes
                .slice(0, 50)
                .map((n) => `- ID: ${n.id}, Title: ${n.title}${n.content ? `, Content: ${n.content.slice(0, 200)}...` : ""}`)
                .join("\n");
            ...
            const systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, "cross_graph_connection_analysis", templateContext, options.userId);
            const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                messages: [
                    { role: "system", content: systemPrompt },
                ...
                maxRetries: 3,
                onRetry: (attempt, error) => {
                    logger.warn(`Cross Graph Connections retry attempt ${attempt}: ${error.message}`);
                },
            });
            const content = completion.choices[0].message.content || "";
            return parseAIResponse(content, "Cross Graph Connections");
        }
        catch (error) {
            const err = error;
    ...
    async generateTaskDetails(title, options = {}) {
        const provider = options.provider
            ? await getAIProvider(options.provider)
            : await getAIProviderForTask("text");
        if (!provider.hasKey) {
        ...
        try {
            return await dedupedRequest(requestKey, async () => {
                const systemPrompt = await promptService.getRenderedPrompt(supabaseAdmin, "generate_task_details", {
                    title,
                ...
                const completion = await withTimeoutAndRetry(() => provider.client.chat.completions.create({
                    messages: [
                        ...
                        {
                            role: "user",
                            content: `任务标题：${title}${options.context ? `\n\n补充信息：${options.context}` : ""}`,
                        },
                    ],
                    model: options.model || provider.model,
                    response_format: { type: "json_object" },
                    ...
                    maxRetries: 3,
                    onRetry: (attempt, error) => {
                        logger.warn(`Generate Task Details retry attempt ${attempt}: ${error.message}`);
                    },
                });
                const content = completion.choices[0].message.content || "";
                const parsed = parseAIResponse(content, "Generate Task Details");
                return {
                    description: parsed.description || "",
                    tags: Array.isArray(parsed.tags) ? parsed.tags.slice(0, 5) : [],
                    estimated_duration: Math.min(180, Math.max(15, parsed.estimated_duration || 25)),
