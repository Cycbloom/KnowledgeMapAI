...
## 概念筛选原则
**优先提取：**
- 核心理论、概念、定义（如"深度学习"、"知识图谱"、"注意力机制"）
- 研究方法、技术手段、算法（如"对比学习"、"迁移学习方法"）
- 关键技术、工具、框架（如"Transformer架构","BERT模型"）
- 重要发现、结论、趋势（如"多模态融合趋势"、"可解释性挑战"）
- 实际应用场景、案例

**严格禁止提取（绝对不要提取以下类型）：**
- **数值指标或度量结果**：包含百分比数字的表述（如"75%模型一致性"、"90%准确率"、"提升23%"）
- **性能度量描述**：任何形式的性能指标名称（如"模型一致性"、"准确率提升"、"召回率提升"、"F1分数"、"AUC值"）
- **研究活动描述**：描述研究过程的短语（如"专题研究"、"实验分析"、"数据预处理"、"文献调研"）
- **简单数值变化**：纯粹的数值对比（如"提升了X%"、"降低了Y倍"、"从A增长到B"）
- **实验细节数据**：具体的参数设置、超数值、样本量等（如"batch_size=32"、"学习率0.001"）
- **泛化描述词**：过于宽泛无实质内容的表述（如"相关研究"、"现有方法"、"本文方法"）
- **重复或相似的概念**

**判断标准（必须同时满足）：**
1. 该概念是否具有独立的知识价值？（能否作为独立知识点学习？）
2. 该概念是否可以在其他文献或场景中复用？（是否具有普遍性？）
3. 该概念是否有助于理解文献的核心思想？（而非仅仅是实验结果？）
4. 该概念是否值得在知识图谱中长期保存？（是否具有持久价值？）
5. 该概念是否不包含具体的数值或百分比？（纯概念 vs 度量结果）

**特别提醒：**
- 如果一个"概念"标题中包含数字或百分号（如"XX%..."、"...X倍"），它很可能是指标而非概念，应直接排除
- "专题研究"、"实验分析"、"数据分析"这类描述研究活动的短语不是概念，是动作描述
- 真正的概念应该是名词性的学术术语，如"卷积神经网络"，而非"神经网络性能提升"
