...
export class PromptService {
     ...
     */
    async getRenderedPrompt(supabase, code, context, userId, graphId) {
     ...
     */
    async getTemplate(supabase, code, userId, graphId) {
        ...
        const relevant = templates.filter(t => {
            if (t.scope === 'system')
                return true;
            if (t.scope === 'user' && t.user_id === userId)
                return true;
            if (t.scope === 'graph' && t.graph_id === graphId)
                return true;
            return false;
        });
    ...
    // Management Methods
    async saveTemplate(supabase, template) {
        const { data, error } = await supabase
            .from('prompt_templates')
            .upsert({
            ...template,
    ...
    async deleteTemplate(supabase, id) {
