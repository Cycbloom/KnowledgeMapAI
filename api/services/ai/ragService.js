...
import { buildNodeContext, buildNodesContext } from './utils.js';
...
export class RAGService {
    ...
    async buildContext(query, userId, options = {}) {
        ...
        const nodesData = searchResults.map(r => ({
            title: r.title,
            content: r.content || ''
        }));
...
${context || '(暂无相关上下文)'}`;
        ...
        try {
            ...
            const suggestedQuestions = await this.generateSuggestedQuestions(message, answer, sources, aiProvider, model);
            return {
                answer,
    ...
    async generateSuggestedQuestions(originalQuestion, answer, sources, provider, model) {
        if (sources.length === 0) {
            return [
                '这个知识点的核心概念是什么？',
                '有哪些相关的知识点？',
...
返回 JSON 格式: { "questions": ["问题1", "问题2", "问题3"] }`
    ...
    async streamChat(message, userId, onChunk, options = {}) {
        const { graphId, currentNodeId, history = [], model } = options;
        const { context, sources } = await this.buildContext(message, userId, {
            graphId,
            currentNodeId
...
${context || '(暂无相关上下文)'}`;
    ...
    async analyzeKnowledgeGaps(graphId, _userId) {
        const { data: graphNodes } = await supabaseAdmin
            .from('graph_nodes')
            .select(`
        knowledge_point_id,
