...
export class JwtService {
    generateToken(user) {
        ...
        const refreshToken = jwt.sign({ ...payload, type: 'refresh' }, secret, { expiresIn: REFRESH_TOKEN_EXPIRES_IN });
        return {
            accessToken,
            refreshToken,
    ...
    refreshAccessToken(refreshToken) {
        ...
        return {
            accessToken: jwt.sign({ userId: payload.userId, email: payload.email }, getSecret(), { expiresIn: ACCESS_TOKEN_EXPIRES_IN }),
            refreshToken,
            expiresIn: ACCESS_TOKEN_EXPIRES_SECONDS,
        };
    }
