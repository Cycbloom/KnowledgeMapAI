...
export const CacheKeys = {
    GRAPH_NODES: (userId, graphId) => `graph_nodes_${userId}_${graphId}`,
    USER_GRAPHS: (userId) => `user_graphs_${userId}`,
    GRAPH: (graphId) => `graph_${graphId}`,
    STUDY_CARDS: (graphId) => `study_cards_${graphId}`,
    TEMPLATES: (category) => `templates_${category}`,
    TEMPLATE: (id) => `template_${id}`,
    PROMPT_TEMPLATE: (code, userId = 'system', graphId = 'none') => `prompt_template_${code}_${userId}_${graphId}`,
    AI_EXPAND: (title, level) => `ai_expand_${title}_${level}`,
    AI_CARDS: (topic, types, count) => `ai_cards_${topic}_${types.sort().join('_')}_${count}`,
    LEARNING_PATH: (graphId) => `learning_path_${graphId}`,
};
const DEFAULT_TTL = 300;
const pendingRequests = new Map();
const stochasticTTL = (baseTTL) => {
...
export const cacheService = {
    ...
    warmup: async (keys) => {
        ...
        const results = await Promise.allSettled(keys.map(async ({ key, fetchFn, ttl }) => {
            const cached = await cacheService.get(key);
            if (!cached) {
        ...
        const skipped = results.filter(r => r.status === 'fulfilled' && r.value.status === 'already_cached').length;
    ...
    invalidateGraphCache: async (userId, graphId) => {
        const keys = [
    ...
    invalidateUserGraphsCache: async (userId) => {
        await cacheService.del(CacheKeys.USER_GRAPHS(userId));
    },
    invalidateStudyCache: async (graphId) => {
        const keys = [
            CacheKeys.STUDY_CARDS(graphId),
            CacheKeys.LEARNING_PATH(graphId),
        ];
