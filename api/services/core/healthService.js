export class HealthService {
    async getOverview(supabase, userId) {
    ...
    async getHeatmap(supabase, userId) {
        const { data: graphs } = await supabase
            .from('graphs')
            .select('id, title')
            .eq('user_id', userId);
      ...
      `)
            .in('graph_id', graphIds)
            .is('deleted_at', null);
        const nodeIds = graphNodes?.map((gn) => gn.knowledge_points?.id || gn.knowledge_point_id) || [];
        const { data: studyCards } = await supabase
            .from('study_cards')
            .select('knowledge_point_id, fsrs_stability, fsrs_difficulty')
            .eq('user_id', userId)
            .in('knowledge_point_id', nodeIds);
        const nodeMastery = new Map();
        if (studyCards) {
            studyCards.forEach(card => {
        ...
        const heatmap = graphNodes?.map((gn) => {
    ...
    async getWeakPoints(supabase, userId) {
        const { data: graphs } = await supabase
            .from('graphs')
            .select('id, title')
            .eq('user_id', userId);
      ...
      `)
            ...
            .is('deleted_at', null);
        const nodeIds = graphNodes?.map((gn) => gn.knowledge_points?.id || gn.knowledge_point_id) || [];
        const nodeMap = new Map(graphNodes?.map((gn) => [gn.knowledge_points?.id || gn.knowledge_point_id, {
    ...
    async getPredictions(supabase, userId) {
        const { data: graphs } = await supabase
            .from('graphs')
            .select('id')
            .eq('user_id', userId);
        ...
        const { data: graphNodes } = await supabase
            ...
            .in('graph_id', graphIds)
            .is('deleted_at', null);
        const nodeIds = graphNodes?.map((gn) => gn.knowledge_point_id) || [];
        const { data: studyCards } = await supabase
            .from('study_cards')
    ...
    async getActivity(supabase, userId, days = 7) {
