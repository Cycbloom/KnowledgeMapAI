...
export class HealthService {
  ...
  async getHeatmap(
    ...
    const heatmap: HeatmapItem[] =
      graphNodes?.map((gn: any) => {
        ...
        const avgMastery =
          masteries.length > 0
            ? masteries.reduce((a: number, b: number) => a + b, 0) /
              masteries.length
            : 0;
  ...
  async getWeakPoints(
    ...
    const { data: studyCards } = await supabase
      .from("study_cards")
      .select(
        "knowledge_point_id, fsrs_stability, fsrs_difficulty, review_count, next_review",
      )
      .eq("user_id", userId)
      .in("knowledge_point_id", nodeIds);
    ...
    if (studyCards) {
      studyCards.forEach((card) => {
        ...
        if (card.next_review) {
          if (
            !stats.nextReview ||
            new Date(card.next_review) < new Date(stats.nextReview)
          ) {
  ...
  async getPredictions(
    ...
    const { data: studyCards } = await supabase
      .from("study_cards")
      .select(
        "knowledge_point_id, fsrs_stability, fsrs_difficulty, next_review",
      )
      .eq("user_id", userId)
      .in("knowledge_point_id", nodeIds);
  ...
  async getActivity(
    ...
    const activityByDay = new Map<
      string,
      { studyTime: number; reviews: number }
    >();
