...
export class CollaboratorService {
    async getCollaborators(supabase, graphId, _userId) {
    ...
    async getUserInfo(supabase, userId) {
    ...
    async inviteCollaborator(supabase, graphId, userId, request) {
        const { data: graph, error: graphError } = await supabase
            .from("knowledge_graphs")
            .select("user_id")
            .eq("id", graphId)
    ...
    async updateCollaboratorRole(supabase, graphId, collaboratorUserId, userId, request) {
        const { data: graph, error: graphError } = await supabase
            .from("knowledge_graphs")
            .select("user_id")
            .eq("id", graphId)
    ...
    async removeCollaborator(supabase, graphId, collaboratorUserId, userId) {
        const { data: graph, error: graphError } = await supabase
            .from("knowledge_graphs")
            .select("user_id")
            .eq("id", graphId)
    ...
    async acceptInvitation(supabase, invitationToken, userId) {
    ...
    async getPendingInvitations(supabase, userId) {
        const { data, error } = await supabase
            .from("graph_collaborators")
            .select(`
        *,
    ...
    async getUserRole(supabase, graphId, userId) {
    ...
    async canEdit(supabase, graphId, userId) {
        const role = await this.getUserRole(supabase, graphId, userId);
        return role === "owner" || role === "editor";
    }
    async canManageCollaborators(supabase, graphId, userId) {
        const role = await this.getUserRole(supabase, graphId, userId);
        return role === "owner";
    }
    async generateShareLink(supabase, graphId, userId, role = "viewer") {
        const { data: graph, error: graphError } = await supabase
            .from("knowledge_graphs")
            .select("user_id")
            .eq("id", graphId)
    ...
    async joinByShareLink(supabase, invitationToken, userId) {
        const { data: invitation, error: inviteError } = await supabase
            .from("graph_collaborators")
            .select("*")
            .eq("invitation_token", invitationToken)
    ...
    async cleanupExpiredInvitations(supabase) {
    ...
    async getInvitationInfo(supabase, invitationToken) {
      ...
      `)
        ...
        return {
            ...
            data: {
                ...
                graphTitle: invitation.graph?.title || "未知图谱",
