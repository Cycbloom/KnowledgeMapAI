...
import type { NodeLevel, ConceptSource, KnowledgePoint } from '../../../shared/types/graph';
...
export class ConceptAggregationService {
  ...
  async findSimilarConcepts(
    ...
    if (kpError || !kp || !kp.embedding) {
      logger.error(`Knowledge point not found or no embedding: ${knowledgePointId}`);
      return [];
    }
    ...
    const { data: candidates, error: candidatesError } = await query.limit(1000);

    if (candidatesError || !candidates) {
      logger.error('Failed to fetch candidate knowledge points:', candidatesError);
      return [];
    }
    ...
    for (const candidate of candidates) {
      ...
      if (similarity >= threshold) {
        const properties = candidate.properties as { sources?: ConceptSource[] } | null;
        results.push({
  ...
  async aggregateConcepts(
    ...
    for (const gn of graphNodes) {
      const kp = gn.knowledge_points as unknown as KnowledgePoint & { embedding?: number[] };
      if (kp && kp.embedding) {
        const properties = kp.properties as { sources?: ConceptSource[] } | undefined;
    ...
    logger.info(`Found ${nodesWithEmbedding.length} nodes with embeddings for aggregation`);
    ...
    for (const group of mergeGroups) {
      ...
      if (!dryRun) {
        ...
        if (updateError) {
          logger.error(`Failed to update primary node ${group.primary.id}:`, updateError);
          continue;
        }
        ...
        for (const duplicate of group.duplicates) {
          ...
          if (duplicateGraphNodes && duplicateGraphNodes.length > 0) {
            ...
            if (edgeUpdateError) {
              logger.error(`Failed to update edges for duplicate ${duplicate.id}:`, edgeUpdateError);
            }
            ...
            if (sourceEdgeUpdateError) {
              logger.error(`Failed to update source edges for duplicate ${duplicate.id}:`, sourceEdgeUpdateError);
            }
            ...
            if (deleteNodeError) {
              logger.error(`Failed to soft delete duplicate graph node ${duplicateGraphNodes[0].id}:`, deleteNodeError);
            }
          }
        }
      }
    ...
    logger.info(`Aggregation complete: ${result.mergedCount} merged, ${result.upgradedNodes.length} upgraded`);
  ...
  async upgradeNodeLevel(
    ...
    const properties = (kp.properties as { sources?: ConceptSource[]; sourceCount?: number }) || {};
    ...
    if (updateError) {
      logger.error(`Failed to update knowledge point ${knowledgePointId}:`, updateError);
      return { success: false };
    }

    logger.info(`Upgraded node ${knowledgePointId} from ${oldLevel} to ${newLevel} (${totalSourceCount} sources)`);
  ...
  async batchCalculateSimilarity(
    ...
    if (error || !kps) {
      logger.error('Failed to fetch knowledge points for batch similarity:', error);
      return result;
    }

    const embeddingMap = new Map<string, { embedding: number[]; title: string; sources: ConceptSource[] }>();
  ...
  async generateEmbeddingsBatch(
    ...
    logger.info(`Batch embedding generation: ${processed} processed, ${failed} failed`);
