...
export class GraphNodeService {
    async addToGraph(supabase, data) {
        const graphNodeData = {
    ...
    async removeFromGraph(supabase, graphNodeId, graphId) {
    ...
    async updatePosition(supabase, graphNodeId, x, y) {
    ...
    async batchUpdatePositions(supabase, positions) {
        if (!positions.length)
            return 0;
    ...
    async updateLevel(supabase, graphNodeId, level) {
        const { data, error } = await supabase
            .from('graph_nodes')
            .update({
            level,
    ...
    async getGraphNodes(supabase, graphId) {
        ...
        return (graphNodes || []).map(gn => buildNodeFromGraphNode(gn));
    }
    async getGraphNodesByKnowledgePoints(supabase, knowledgePointIds) {
    ...
    async batchDelete(supabase, graphNodeIds, graphId) {
...
//# sourceMappingURL=graphNodeService.js.map