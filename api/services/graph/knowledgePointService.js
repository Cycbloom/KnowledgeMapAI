...
export class KnowledgePointService {
    ...
    async searchSimilar(supabase, embedding, userId, threshold = 0.85, limit = 10) {
    ...
    async listAccessible(supabase, userId) {
        const { data, error } = await supabase
            .from('knowledge_points')
            .select('*')
    ...
    async getGraphs(supabase, knowledgePointId, userId) {
    ...
    async getAccessible(supabase, id, userId) {
        const { data, error } = await supabase
            .from('knowledge_points')
            .select('*')
            .eq('id', id)
    ...
    async checkOwnership(supabase, id, userId) {
        const { data, error } = await supabase
            .from('knowledge_points')
            .select('owner_id')
            .eq('id', id)
    ...
    async listPublic(supabase, options) {
    ...
    async submitForPublic(supabase, options, userId) {
        ...
        try {
            const tags = kp.properties?.tags?.join(', ') || '';
            ...
            if (textToSearch) {
                ...
                const publicDuplicates = similarKps.filter((skp) => skp.id !== knowledge_point_id && skp.visibility === 'public');
    ...
    async listPending(supabase, options) {
    ...
    async approvePublic(supabase, knowledgePointId, adminUserId) {
        ...
        if (suggestedChanges) {
            if (suggestedChanges.title)
                updates.title = suggestedChanges.title;
            if (suggestedChanges.content)
                updates.content = suggestedChanges.content;
            if (suggestedChanges.learning_material)
                updates.learning_material = suggestedChanges.learning_material;
        }
        updates.properties = {
            ...kp.properties,
    ...
    async rejectPublic(supabase, knowledgePointId, adminUserId, reason) {
        const kp = await this.get(supabase, knowledgePointId);
        if (!kp || kp.visibility !== 'pending') {
            throw new AppError(ErrorCodes.RESOURCE_NOT_FOUND);
        }
...
//# sourceMappingURL=knowledgePointService.js.map