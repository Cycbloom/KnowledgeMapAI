...
export class KnowledgePointVersionService {
    async listVersions(supabase, knowledgePointId, options) {
    ...
    async getVersion(supabase, knowledgePointId, versionNumber) {
    ...
    async getLatestVersion(supabase, knowledgePointId) {
        const { data, error } = await supabase
            .from('knowledge_point_versions')
            .select('*')
            .eq('knowledge_point_id', knowledgePointId)
    ...
    async compareVersions(supabase, knowledgePointId, versionNumber1, versionNumber2) {
    ...
    computeDiffs(older, newer) {
        const diffs = [];
        const fields = ['title', 'content', 'learning_material', 'properties'];
    ...
    async rollback(supabase, knowledgePointId, versionNumber, _userId) {
    ...
    async getVersionHistory(supabase, knowledgePointId, options) {
    ...
    async createManualVersion(supabase, knowledgePointId, changeSummary, userId) {
        const kp = await supabase
            .from('knowledge_points')
            .select('*')
            .eq('id', knowledgePointId)
...
//# sourceMappingURL=knowledgePointVersionService.js.map