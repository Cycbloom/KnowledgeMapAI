...
export class DependencyResolver {
  resolve(plugins: Plugin[]): Plugin[] {
    ...
    if (sorted.length !== allNames.size) {
      // 预构建 sorted 集合，替代 filter 内 sorted.includes 的 O(n²) 扫描
    ...
    logger.info(
      `[DependencyResolver] Resolved activation order: ${sorted.join(" → ")}`,
    );

    return sorted.map((name) => pluginMap.get(name) as Plugin);
  }
}
