...
export class FocusService {
    async createFocusSession(client, userId, data) {
    ...
    async updateFocusSession(client, sessionId, userId, updates) {
    ...
    async getFocusSessions(client, userId, options) {
        let query = client
            .from("focus_sessions")
            .select("*")
            .eq("user_id", userId)
    ...
    async getUserFocusStats(client, userId) {
        const { data, error } = await client
    ...
    async getDailyFocusStats(client, userId, date) {
    ...
    async getWeeklyFocusStats(client, userId, weekStart) {
        const now = new Date();
        ...
        const totalDuration = sessions?.reduce((sum, s) => sum + (s.duration || 0), 0) || 0;
        const bestDay = Object.entries(dailyStats).reduce((best, [date, duration]) => duration > best.duration ? { date, duration } : best, { date: "", duration: 0 });
        return {
    ...
    async getMonthlyFocusStats(client, userId, year, month) {
        const now = new Date();
    ...
    async getYearlyHeatmap(client, userId, year) {
