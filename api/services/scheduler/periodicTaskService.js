...
export class PeriodicTaskService {
    async initPeriodicTasks(userId) {
    ...
    async initPass(userId, periodType, start, end) {
    ...
    async updatePeriodicTaskProgress(userId, taskType, currentValue) {
        const periodTypes = ['weekly', 'monthly', 'quarterly'];
    ...
    async getPassProgress(userId) {
        ...
        for (const periodType of ['weekly', 'monthly', 'quarterly']) {
            const { start } = getPeriodDates(periodType);
    ...
    async claimPassReward(userId, passId, level) {
        const { data: pass } = await supabaseAdmin
            .from('periodic_passes')
            .select('*')
            .eq('id', passId)
    ...
    async checkDailyTaskStreak(userId) {
    ...
    async checkPeriodicStreak(userId) {
        const periodTypes = ['weekly', 'monthly', 'quarterly'];
        for (const periodType of periodTypes) {
            const { start, end } = getPeriodDates(periodType);
            const today = new Date().toISOString().split('T')[0];
            ...
            const { data: stats } = await supabaseAdmin
                ...
                .eq('user_id', userId)
                .single();
            const streakColumn = `${periodType}_streak`;
            if (allCompleted) {
                const newStreak = (stats?.[streakColumn] || 0) + 1;
                await supabaseAdmin
                    .from('user_focus_stats')
    ...
    async checkAndUnlockAchievement(userId, conditionType, value) {
...
//# sourceMappingURL=periodicTaskService.js.map