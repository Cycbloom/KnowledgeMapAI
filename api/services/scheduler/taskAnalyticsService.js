export class TaskAnalyticsService {
    async getAnalytics(client, userId) {
        const now = new Date();
        ...
        const todayCompleted = completedTasks.filter((t) => new Date(t.completed_at) >= today).length;
        const weekCompleted = completedTasks.filter((t) => new Date(t.completed_at) >= weekAgo).length;
        const monthCompleted = completedTasks.filter((t) => new Date(t.completed_at) >= monthAgo).length;
        ...
        const comparison = await this.calculateComparison(client, userId, allTasks, weekAgo);
        return {
            overview: {
                todayCompleted,
                weekCompleted,
    ...
    calculateCompletionTrend(tasks, days) {
        const trend = [];
        const now = new Date();
        let cumulative = 0;
    ...
    calculateTimeDistribution(executions) {
        const distribution = [];
        const map = {};
    ...
    calculateQueueStats(tasks) {
        ...
        for (let level = 0; level <= 2; level++) {
            ...
            stats.push({
                ...
                completionRate: queueTasks.length > 0
        ...
        for (let p = 1; p <= 4; p++) {
            ...
            completed.forEach((task) => {
                if (task.deadline && task.completed_at) {
                    ...
                    const delay = (completedAt.getTime() - deadline.getTime()) / (1000 * 60 * 60);
                    if (delay > 0) {
                        totalDelay += delay;
                        delayCount++;
                    }
    ...
    async calculateComparison(_client, _userId, currentTasks, weekAgo) {
        ...
        const completedChange = previousCount > 0
    ...
    async generateInsights(client, userId) {
        ...
        const bestQueue = analytics.queueStats.reduce((best, q) => q.completionRate > best.completionRate ? q : best);
        if (bestQueue.completionRate > 0) {
            insights.push({
                type: "positive",
                title: `Q${bestQueue.queueLevel}队列效率最高`,
                description: `完成率达到 ${bestQueue.completionRate}%`,
                recommendation: bestQueue.queueLevel === 0
                    ? "专注模式效果显著，建议多使用"
                    : undefined,
            });
        }
        const highPriorityTasks = analytics.priorityStats.find((p) => p.priority === 4);
...
//# sourceMappingURL=taskAnalyticsService.js.map