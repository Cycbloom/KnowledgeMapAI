...
export class TaskRecommendationService {
    calculateUrgencyScore(task, now = new Date()) {
        let score = 0;
        if (task.deadline) {
    ...
    async calculateEfficiencyData(client, userId, days = 30) {
      ...
      `)
        ...
        for (const exec of executions || []) {
            ...
            const task = exec.scheduled_tasks;
            if (task?.tags) {
        ...
        const processedQueueEfficiency = {};
        ...
        const sortedHours = Object.entries(avgHourlyEfficiency).sort(([, a], [, b]) => b - a);
    ...
    getTimeSlotRecommendations(tasks, timeSlot) {
    ...
    async getTaskRecommendations(client, userId, context) {
        ...
        const recommendations = tasks.map((task) => {
            ...
            if (task.tags && task.tags.length > 0) {
                const matchingTags = task.tags.filter((tag) => TIME_SLOT_CONFIG[currentTimeSlot.type].recommendedTypes.includes(tag));
            ...
            if (tagEfficiencies.length > 0) {
                const avgEfficiency = tagEfficiencies.reduce((a, b) => a + b, 0) /
    ...
    analyzePriorityFromText(title, description) {
    ...
    async getSmartSuggestions(client, userId, context) {
        ...
        const bestQueue = Object.entries(efficiencyData.queueEfficiency).sort(([, a], [, b]) => b.completionRate - a.completionRate)[0];
        ...
        if (bestTags.length > 0) {
            efficiencyTips.push(`您在以下类型任务表现较好: ${bestTags.map(([tag]) => tag).join("、")}`);
        }
        if (efficiencyData.peakHours.length > 0) {
            const formatHour = (h) => `${h}:00`;
            efficiencyTips.push(`您的效率高峰时段: ${efficiencyData.peakHours.map(formatHour).join("、")}`);
        }
        return {
            topTasks,
    ...
    calculateOptimalTaskOrder(tasks, efficiencyData, now = new Date()) {
    ...
    async checkTaskDependencies(client, taskId, _userId) {
    ...
    async getSmartRecommendation(client, userId, context) {
        const now = context?.currentTime || new Date();
        const recommendations = await this.getTaskRecommendations(client, userId, context);
        const efficiencyData = await this.calculateEfficiencyData(client, userId);
        const currentTimeSlot = this.getCurrentTimeSlot(now);
        ...
        const efficiencyLevel = isPeakHour
            ? "high"
        ...
        if (recommendedTask) {
            reasons.push(...this.generateRecommendationReasons(recommendedTask, currentTimeSlot, isPeakHour, efficiencyData));
        }
        return {
            recommendedTask,
    ...
    generateRecommendationReasons(recommendation, timeSlot, isPeakHour, efficiencyData) {
        const reasons = [];
    ...
    calculateDynamicPriority(task, now = new Date()) {
        const factors = [];
        let totalScore = 50;
        if (task.deadline) {
            const deadline = new Date(task.deadline);
...
//# sourceMappingURL=taskRecommendationService.js.map