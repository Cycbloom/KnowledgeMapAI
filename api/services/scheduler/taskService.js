...
export class TaskService {
    async createTask(client, userId, taskData) {
        ...
        const { data, error } = await client
            ...
            .single();
        if (error)
            throw new AppError(ErrorCodes.SCHEDULER_TASK_CREATION_FAILED, { details: { originalError: error.message } });
        return data;
    }
    async updateTask(client, taskId, userId, updates) {
        const { data, error } = await client
            .from("scheduled_tasks")
            .update({
            ...updates,
    ...
    async deleteTask(client, taskId, userId) {
    ...
    async getTask(client, taskId, userId) {
        const { data, error } = await client
            .from("scheduled_tasks")
            .select("*")
            .eq("id", taskId)
    ...
    async getTasks(client, userId, filters, options) {
    ...
    async getTasksByQueue(client, userId, queueLevel) {
        const { data, error } = await client
            .from("scheduled_tasks")
            .select("*")
            .eq("user_id", userId)
    ...
    async startTask(client, taskId, userId) {
    ...
    async pauseTask(client, taskId, userId) {
        const { data: task, error: taskError } = await client
            .from("scheduled_tasks")
            .update({
            status: "paused",
    ...
    async completeTask(client, taskId, userId) {
    ...
    async moveTaskToQueue(client, taskId, userId, targetQueue) {
        const { data: maxPosResult } = await client
            .from("scheduled_tasks")
            .select("position")
            .eq("user_id", userId)
    ...
    async reorderTasks(client, userId, queueLevel, taskIds) {
    ...
    async demoteTask(client, taskId, userId) {
    ...
    async getTimeSlice(client, userId, queueLevel) {
