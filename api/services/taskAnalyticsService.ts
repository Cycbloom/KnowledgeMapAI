...
export class TaskAnalyticsService {
  async getAnalytics(client: SupabaseClient, userId: string): Promise<TaskAnalytics> {
    const now = new Date();
  ...
  private calculateCompletionTrend(tasks: any[], days: number) {
    const trend: Array<{ date: string; completed: number; total: number; cumulative: number }> = [];
    const now = new Date();
    let cumulative = 0;
  ...
  async generateInsights(client: SupabaseClient, userId: string): Promise<Insight[]> {
  ...
  private findPeakHours(distribution: Array<{ day: number; hour: number; value: number }>): number[] {
