...
export class InfiniteExpansionProcessor {
    async process(taskId, userId, payload, supabase, updateTaskStatus) {
        try {
            ...
            const { source_graph_id, source_graph_title, source_graph_description, max_depth = 2, max_graphs_per_level = 3, relation_types = ['prerequisite', 'extension', 'related'], auto_generate_nodes = true, node_depth = 2 } = payload;
            ...
            while (queue.length > 0) {
                ...
                const systemPrompt = await promptService.getRenderedPrompt(supabase, 'infinite_graph_expansion', {
                        ...
                        if (duplicateCheck.isDuplicate && duplicateCheck.similarGraphs[0]) {
                            ...
                            if (newGraph) {
                                ...
                                if (auto_generate_nodes && targetGraphId) {
                                    ...
                                    const nodeCount = await generateNodesForGraph(supabase, targetGraphId, suggestion.title, suggestion.description, node_depth, provider, userId);
...
//# sourceMappingURL=infiniteExpansionProcessor.js.map