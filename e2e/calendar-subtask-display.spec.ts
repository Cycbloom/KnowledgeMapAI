...
class CalendarPageObject {
  ...
  async getMonthViewButton() {
    return this.page.locator('button:has-text("月"), button:has-text("month")').first();
  }

  async getWeekViewButton() {
    return this.page.locator('button:has-text("周"), button:has-text("week")').first();
  }

  async getDayViewButton() {
    return this.page.locator('button:has-text("日"), button:has-text("day")').first();
  }

  async getScheduleViewButton() {
    return this.page.locator('button:has-text("日程"), button:has-text("schedule")').first();
  }
  ...
  async getShowSubtasksToggle() {
    return this.page.locator('button:has-text("显示子任务"), button:has-text("隐藏子任务")').first();
  }
  ...
  async isSubtaskToggleActive(): Promise<boolean> {
    ...
    return className?.includes('primary') || className?.includes('active') || false;
  }

  async getSubtaskStack() {
    return this.page.locator('[data-testid="calendar-subtask-stack"], .calendar-subtask-stack');
  }
  ...
  async getSubtaskDetailModal() {
    return this.page.locator('[data-testid="subtask-detail-modal"], .subtask-detail-modal');
  }
  ...
  async navigateToDate(date: Date) {
    ...
    const prevButton = this.page.locator('button[aria-label="上一月"], button:has-text("‹")').first();
    const nextButton = this.page.locator('button[aria-label="下一月"], button:has-text("›")').first();
  ...
  async getTodayButton() {
    return this.page.locator('button:has-text("今天"), button:has-text("today")').first();
  }
  ...
  async createTask(title: string, description?: string) {
    const createButton = this.page.locator('button:has-text("添加任务"), button:has-text("新建")').first();
  ...
  async getSubtaskProgress(): Promise<{ completed: number; total: number } | null> {
  ...
  async getSubtaskMasteryBadge(): Promise<string | null> {
    const masteryBadge = this.page.locator('[data-testid="mastery-badge"], .mastery-badge').first();
    return masteryBadge.textContent();
  }

  async getLearningStateBadge(): Promise<string | null> {
    const stateBadge = this.page.locator('[data-testid="learning-state-badge"], .learning-state-badge').first();
    return stateBadge.textContent();
  }

  async expandSubtaskStack() {
    const expandButton = this.page.locator('button:has-text("更多"), button:has-text("+")').first();
...
test.describe('日历页面基本功能测试', () => {
  ...
  test('应该能够切换视图', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="week-view"], .week-view').first()).toBeVisible({ timeout: 5000 });
    
    await calendarPage.switchToDayView();
    await expect(page.locator('[data-testid="day-view"], .day-view').first()).toBeVisible({ timeout: 5000 });
    
    await calendarPage.switchToMonthView();
    await expect(page.locator('[data-testid="month-view"], .month-view').first()).toBeVisible({ timeout: 5000 });
  });
...
test.describe('子任务显示开关测试', () => {
  ...
  test('应该能够隐藏子任务', async ({ page }) => {
    ...
    const isVisible = await subtaskStack.first().isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('日历子任务堆叠显示测试', () => {
  ...
  test('应该在月视图中显示子任务堆叠', async ({ page }) => {
    ...
    const eventWithSubtasks = page.locator('[data-has-subtasks="true"], .has-subtasks').first();
    const hasEventWithSubtasks = await eventWithSubtasks.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该在周视图中显示子任务堆叠', async ({ page }) => {
    ...
    const subtaskStack = await calendarPage.getSubtaskStack();
    const hasSubtaskStack = await subtaskStack.first().isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('子任务状态显示测试', () => {
  ...
  test('应该显示子任务的学习状态徽章', async ({ page }) => {
    ...
    const stateBadge = page.locator('[data-testid="learning-state-badge"], .learning-state-badge').first();
    const isVisible = await stateBadge.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该显示子任务的掌握度百分比', async ({ page }) => {
    ...
    const isVisible = await masteryText.isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('日历事件与子任务关联测试', () => {
  ...
  test('应该在事件卡片上显示子任务数量', async ({ page }) => {
    ...
    const subtaskCountBadge = page.locator('text=/\\d+\\/\\d+/, [data-subtask-count]').first();
    const isVisible = await subtaskCountBadge.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该点击事件时显示关联的子任务', async ({ page }) => {
    ...
    const eventCard = page.locator('[data-testid="calendar-event"], .calendar-event').first();
    if (await eventCard.isVisible({ timeout: 3000 }).catch(() => false)) {
      await eventCard.click();
      
      const subtaskSection = page.locator('text=/子任务|subtask/i').first();
      const hasSubtaskSection = await subtaskSection.isVisible({ timeout: 5000 }).catch(() => false);
