...
class MasteryPageObject {
  ...
  async getReviewTaskCount(): Promise<number> {
    const reviewCards = this.page.locator('[data-testid="review-task-card"], .review-task-card');
  ...
  async completeReview(quality: 'again' | 'hard' | 'good' | 'easy') {
    const qualityButton = this.page.locator(`button:has-text("${quality}"), button[data-quality="${quality}"]`).first();
  ...
  async getKnowledgePointMastery(): Promise<number> {
    const masteryBar = this.page.locator('[data-testid="mastery-progress"], .mastery-progress').first();
  ...
  async triggerManualDecay() {
    const decayButton = this.page.locator('button:has-text("计算衰减"), button:has-text("刷新掌握度")').first();
...
test.describe('掌握度衰减测试', () => {
  ...
  test('应该显示知识点掌握度', async ({ page }) => {
    ...
    const masteryIndicator = page.locator('[data-testid="mastery-indicator"], .mastery-level, text=/掌握度|%/').first();
  ...
  test('应该根据掌握度显示不同颜色', async ({ page }) => {
    ...
    const lowMastery = page.locator('.mastery-low, [data-mastery="low"]').first();
    const mediumMastery = page.locator('.mastery-medium, [data-mastery="medium"]').first();
    const highMastery = page.locator('.mastery-high, [data-mastery="high"]').first();
...
test.describe('复习提醒触发测试', () => {
  ...
  test('应该显示待复习知识点数量', async ({ page }) => {
    ...
    const countBadge = page.locator('[data-testid="review-count"], .review-count, text=/\\d+.*待复习/').first();
  ...
  test('应该显示复习任务的紧迫程度', async ({ page }) => {
    ...
    const urgencyIndicator = page.locator('[data-testid="urgency-badge"], .urgency-badge, text=/紧急|urgent|逾期|overdue/').first();
...
test.describe('SM2 算法测试', () => {
  ...
  test('应该显示下次复习日期', async ({ page }) => {
    ...
    const nextReviewDate = page.locator('text=/下次复习|next review|\\d+天/i').first();
  ...
  test('应该显示易遗忘因子', async ({ page }) => {
    ...
    const isVisible = await easeFactorInfo.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该能够完成复习并更新间隔', async ({ page }) => {
    ...
    const reviewCard = page.locator('[data-testid="review-task-card"], .review-task-card').first();
    if (await reviewCard.isVisible({ timeout: 3000 }).catch(() => false)) {
      await reviewCard.click();
      
      const qualityButton = page.locator('button:has-text("良好"), button:has-text("good"), button[data-quality="good"]').first();
      if (await qualityButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await qualityButton.click();
        
        const successMessage = page.locator('text=/复习完成|completed|更新成功/i');
...
test.describe('掌握度与子任务同步测试', () => {
  ...
  test('应该同步知识点掌握度到子任务', async ({ page }) => {
    ...
    const knowledgePointCard = page.locator('[data-testid="knowledge-point-card"], .knowledge-point-card').first();
    if (await knowledgePointCard.isVisible({ timeout: 3000 }).catch(() => false)) {
...
test.describe('掌握度衰减计算测试', () => {
  ...
  test('应该计算并显示衰减后的掌握度', async ({ page }) => {
    ...
    const lastStudyInfo = page.locator('text=/上次学习|last study|\\d+天前/i').first();
    const isVisible = await lastStudyInfo.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该标记需要复习的知识点', async ({ page }) => {
    ...
    const needsReviewBadge = page.locator('[data-testid="needs-review"], .needs-review, text=/需要复习|needs review/i').first();
    const isVisible = await needsReviewBadge.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('应该显示预计遗忘时间', async ({ page }) => {
    ...
    const forgetPrediction = page.locator('text=/预计遗忘|forget prediction|\\d+天后/i').first();
    const isVisible = await forgetPrediction.isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('批量复习操作测试', () => {
  ...
  test('应该能够批量开始复习', async ({ page }) => {
    ...
    const startReviewButton = page.locator('button:has-text("开始复习"), button:has-text("start review")').first();
    if (await startReviewButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      await startReviewButton.click();
      
      const reviewInterface = page.locator('[data-testid="review-interface"], .review-interface').first();
      await expect(reviewInterface).toBeVisible({ timeout: 5000 }).catch(() => {});
    }
  ...
  test('应该显示复习统计信息', async ({ page }) => {
    ...
    const statsSection = page.locator('text=/统计|statistics|复习次数/i').first();
