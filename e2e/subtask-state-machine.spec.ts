...
class SubtaskPageObject {
  ...
  async getSubtaskStatus(subtaskTitle: string): Promise<string | null> {
    const subtaskContainer = this.page
      .locator(`text="${subtaskTitle}"`)
      .first()
      .locator("..");
    const statusBadge = subtaskContainer
  ...
  async transitionSubtaskState(
    toState: "review" | "practice" | "quiz",
    masteryLevel: number,
  ) {
...
test.describe("子任务状态转换流程测试", () => {
  ...
  test("应该从 learning 状态转换到 review 状态（掌握度 < 30%）", async ({
    page,
  }) => {
    await subtaskPage.openTaskDetail(testTaskTitle);
  ...
  test("应该从 learning 状态转换到 practice 状态（掌握度 30%-70%）", async ({
    page,
  }) => {
  ...
  test("应该从 learning 状态转换到 quiz 状态（掌握度 > 70%）", async ({
    page,
  }) => {
...
test.describe("子任务状态循环测试", () => {
  ...
  test("应该完成 review → practice 状态转换", async ({ page }) => {
    ...
    await expect(page.locator("text=复习").first()).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(page.locator("text=练习").first()).toBeVisible({
      timeout: 5000,
    });
  });

  test("应该完成 practice → quiz 状态转换（掌握度 >= 50%）", async ({
    page,
  }) => {
    ...
    await expect(page.locator("text=测验").first()).toBeVisible({
      timeout: 5000,
    });
  });

  test("应该完成 practice → review 状态转换（掌握度 < 50%）", async ({
    page,
  }) => {
  ...
  test("应该完成 quiz → practice 状态转换（掌握度 60%-80%）", async ({
    page,
  }) => {
  ...
  test("应该完成完整的状态循环 review → practice → quiz → review", async ({
    page,
  }) => {
