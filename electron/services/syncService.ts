...
export class SyncService {
  ...
  async sync(): Promise<boolean> {
    ...
    try {
      ...
      for (const device of this.deviceDiscoveryService
        .getDevices()
        .filter((d: SyncDevice) => d.status === "online")) {
  ...
  private async processConflicts(): Promise<void> {
    ...
    const resolvedOperations = this.conflictService.autoResolveConflicts(
      this.conflicts,
    );
  ...
  resolveConflict(
    ...
    if (conflict) {
      ...
      const resolvedOperation = this.conflictService.resolveConflict(
        conflict,
        resolution,
      );
      ...
      console.log(
        `Resolved conflict ${conflictId} with ${resolution} resolution`,
      );
    }
  }
...
export const syncService = new SyncService(
  path.join(process.env.APPDATA || "", "KnowledgeMap", "knowledgemap.db"),
);
