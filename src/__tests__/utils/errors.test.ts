...
describe('errors utilities', () => {
  describe('AppError', () => {
    ...
    it('should create error with custom values', () => {
      ...
      const error = new AppError('Custom error', 'AUTH_ERROR', 401, context, false);
    ...
    it('should deserialize from JSON', () => {
      const original = new AppError('Test error', 'AUTH_ERROR', 401, { userId: 123 });
  ...
  describe('getErrorMessage', () => {
    it('should return message from AppError', () => {
      expect(getErrorMessage(new AppError('Custom message'))).toBe('Custom message');
    });
  ...
  describe('getUserFriendlyMessage', () => {
    ...
    it('should return default message for non-AppError', () => {
      expect(getUserFriendlyMessage(new Error('test'))).toBe('操作失败，请稍后重试');
    });
  });

  describe('createErrorFromResponse', () => {
    it('should create NetworkError for status 0', () => {
      const error = createErrorFromResponse({ status: 0, statusText: 'Network Error' });
      expect(error.name).toBe('NetworkError');
      expect(error.code).toBe('NETWORK_ERROR');
    });
    ...
    it('should create ValidationError for status 400', () => {
      const response = {
        ...
        data: { message: 'Invalid data', details: [{ field: 'email', message: 'Invalid' }] },
      };
    ...
    it('should create ServerError for 5xx status', () => {
      ...
      const error503 = createErrorFromResponse({ status: 503, statusText: 'Service Unavailable' });
      expect(error503.name).toBe('ServerError');
      
      const error504 = createErrorFromResponse({ status: 504, statusText: 'Gateway Timeout' });
      expect(error504.name).toBe('ServerError');
    });

    it('should create AppError for unknown status', () => {
      const error = createErrorFromResponse({ status: 418, statusText: "I'm a teapot" });
      expect(error.name).toBe('AppError');
      expect(error.code).toBe('UNKNOWN_ERROR');
