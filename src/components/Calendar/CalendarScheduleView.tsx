...
import { Plus, Clock, CheckCircle, ChevronLeft, ChevronRight, Move } from 'lucide-react';
...
export const CalendarScheduleView: React.FC<CalendarScheduleViewProps> = ({
  ...
  const [hoveredSlot, setHoveredSlot] = useState<{ hour: number; y: number } | null>(null);
  ...
  const scheduleData = useMemo(() => {
    ...
    const matchedExecutions = dayExecutions.filter((exec) => {
      return dayEvents.some((event) => {
        ...
        return timeDiff < 30 * 60 * 1000 && event.title.toLowerCase().includes(exec.task_title.toLowerCase());
      });
    });
    ...
    return {
      ...
      stats: {
        ...
        matchRate: dayEvents.length > 0 ? Math.round((matchedExecutions.length / dayEvents.length) * 100) : 0,
      },
    };
  ...
  const getEventColor = (event: CalendarEvent) => {
    switch (event.type) {
      ...
      default:
        return { bg: 'bg-gray-500', border: 'border-gray-600', light: 'bg-gray-100 dark:bg-gray-500/20' };
    }
  };
        ...
        <div className="relative min-h-[1920px]">
          ...
          {scheduleData.slots.map((slot) => (
            <div
              ...
              onClick={() => !draggedEvent && onAddEvent(currentDate, slot.hour)}
            >
              {/* Time label */}
              ...
              {/* Content area with planned and executed */}
              <div className="flex-1 relative p-1 flex gap-1">
                ...
                <div className="flex-1 space-y-1">
                  {slot.planned.map((event, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          {onEventDrop && <Move size={12} className="opacity-50" />}
                          <Clock size={12} />
                ...
                {/* Add button on hover */}
                {hoveredSlot?.hour === slot.hour && !slot.planned.length && !slot.executed.length && !draggedEvent && (
