...
export const NodeDetailSidebar: React.FC<NodeDetailSidebarProps> = ({
  ...
  return (
    <div className="h-full flex flex-col">
      <div className={`flex justify-between items-center ${isMobile ? 'mb-4 px-2' : 'mb-6'}`}>
        <div className="flex items-center space-x-2">
          ...
          <div className={`rounded-full ${isMobile ? 'w-4 h-4' : 'w-3 h-3'}`} style={{ backgroundColor: colors.primary }}></div>
          <h3 className={`font-bold text-gray-800 dark:text-gray-100 ${isMobile ? 'text-xl' : 'text-lg'}`}>节点详情</h3>
        </div>
        <button 
          onClick={onClose} 
      ...
      <div className={`flex-1 space-y-6 overflow-y-auto ${isMobile ? 'pr-0 px-2 pb-24' : 'pr-1'}`}>
        <section>
          <h1 className={`font-bold text-gray-900 dark:text-gray-100 leading-tight ${isMobile ? 'text-xl mb-4' : 'text-2xl mb-3'}`}>{node.title}</h1>
          
          {/* Metadata Row */}
          <div className={`flex flex-wrap gap-2 text-xs text-gray-500 dark:text-gray-400 ${isMobile ? 'mb-4' : 'gap-3 mb-4'}`}>
            <div className={`flex items-center bg-gray-50 dark:bg-gray-800 rounded ${isMobile ? 'px-2.5 py-1.5' : 'px-2 py-1'}`}>
            ...
            {isMastered ? (
                <div className={`flex items-center bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded border border-green-100 dark:border-green-800 ${isMobile ? 'px-2.5 py-1.5' : 'px-2 py-1'}`}>
                  <Check size={14} className="mr-1" /> 已掌握
                </div>
              ) : (
                 <div className={`flex items-center bg-gray-50 dark:bg-gray-800 rounded ${isMobile ? 'px-2.5 py-1.5' : 'px-2 py-1'}`}>
                   <div className={`rounded-full mr-1.5 ${isMobile ? 'w-2.5 h-2.5' : 'w-2 h-2'}`} style={{ backgroundColor: colors.primary }} />
                   {status === 'new' ? '未开始' : '学习中'}
                 </div>
              )}
          </div>
        ...
        {/* Learning Actions */}
        <section className={`grid gap-3 ${isMobile ? 'grid-cols-1' : 'grid-cols-2'}`}>
          <button 
          ...
          >
            ...
            <span className={`font-bold ${isMobile ? 'text-base' : ''}`}>开启沉浸学习</span>
          </button>
          ...
          >
            <GraduationCap size={isMobile ? 20 : 18} className="mr-2 text-indigo-500" />
          ...
          <button 
            ...
            className={`flex items-center justify-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${isMobile ? 'p-3.5 min-h-[48px]' : 'p-3'}`}
          >
            <Sparkles size={isMobile ? 20 : 18} className="mr-2 text-amber-500" />
        ...
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-2xl p-4 border border-purple-100 dark:border-purple-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className={`font-bold text-purple-900 dark:text-purple-300 flex items-center ${isMobile ? 'text-base' : ''}`}>
              <Wand2 size={isMobile ? 18 : 16} className="mr-2" />
              AI 深度探索
            </h3>
            <span className="text-[10px] bg-purple-200 dark:bg-purple-800 text-purple-800 dark:text-purple-300 px-2 py-0.5 rounded-full">Beta</span>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-purple-200 dark:border-purple-800">
           <div className="flex justify-between items-center mb-2">
             <h5 className="text-xs font-bold text-purple-700 dark:text-purple-400">🔗 语义相关节点</h5>
             {!showRelatedSection && (
               <button 
