  ...
  const generateScript = async () => {
    ...
    try {
      ...
      const priorityNodes = [...nodes].sort((a, b) => {
        ...
        return (levels[a.level || 'normal'] || 3) - (levels[b.level || 'normal'] || 3);
      });
          ...
          <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-slate-800 dark:to-slate-800">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-full">
                <Volume2 className="text-purple-600 dark:text-purple-400" size={24} />
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">AI 知识播客</h2>
                <p className="text-xs text-slate-500 dark:text-slate-400">为您深度解读 "{graphTitle}"</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-hidden flex flex-col relative">
            {isGenerating ? (
              <div className="flex-1 flex flex-col items-center justify-center space-y-4 p-8">
                ...
                <p className="text-slate-600 dark:text-slate-300 animate-pulse">正在为您撰写播客脚本...</p>
