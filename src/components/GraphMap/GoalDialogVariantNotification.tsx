...
import {
  ...
  type GoalDialogVariantNotice,
} from '../../store/useGoalDialogVariantNotificationStore';
...
export const GoalDialogVariantNotificationBridge: React.FC<{
  onClose: () => void;
  onContinue: () => void;
}> = ({ onClose, onContinue }) => {
  const notice = useGoalDialogVariantNotificationStore((s) => s.notice);
  ...
  useEffect(() => {
    if (notice) {
      ...
      useNotificationManager.getState().dismiss('goal-variant');
    }
  }, [notice, onClose, onContinue]);
