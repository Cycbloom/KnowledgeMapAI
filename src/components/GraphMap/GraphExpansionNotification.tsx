...
import {
  useGraphExpansionNotificationStore,
  type GraphExpansionNotice,
} from '../../store/useGraphExpansionNotificationStore';
...
> = ({ notice, onClose, onContinue }) => {
  ...
  return (
    <div
      className={cn(
        'w-80 rounded-lg shadow-xl border backdrop-blur-sm',
        isDark
...
export const GraphExpansionNotificationBridge: React.FC<{
  onClose: () => void;
  onContinue: () => void;
}> = ({ onClose, onContinue }) => {
  const notice = useGraphExpansionNotificationStore((s) => s.notice);

  useEffect(() => {
    if (notice) {
      useNotificationManager.getState().show({
        id: 'graph-expansion',
        kind: 'graph-expansion',
        priority: 20,
        status: notice.status,
        dismissible: true,
        data: notice,
        action: notice.status === 'success' ? onContinue : undefined,
        onDismiss: onClose,
      });
    } else {
      useNotificationManager.getState().dismiss('graph-expansion');
    }
  }, [notice, onClose, onContinue]);
