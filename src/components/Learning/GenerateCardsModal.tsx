...
export const GenerateCardsModal: React.FC<GenerateCardsModalProps> = ({
  ...
  // 预构建已选题型集合，避免渲染题型列表时对每个选项线性 includes（原为 O(cardTypes*types)）
  ...
  return (
    ...
    >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {cardTypes.map(type => (
                ...
                >
                  {type.label}
                  {typeSet.has(type.id) && <div className="w-1.5 h-1.5 rounded-full bg-primary-500" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="px-2">
              <input
                ...
                aria-valuetext={t('learning.generateCards.countUnit', { count })}
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 flex justify-end gap-3" aria-busy={isGenerating}>
          {isMobile && isGenerating && generateProgress ? (
            ...
            <>
              ...
              {isMobile && !isMobileAIConfigured ? (
                ...
                >
                  {isLoading ? <Loader2 size={18} className="animate-spin" aria-hidden="true" /> : <Sparkles size={18} aria-hidden="true" />}
