...
import { PROMPT_SCENARIOS, getScenarioById, type PromptScenario } from '../PromptConfig/promptScenarios';
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] w-full max-w-4xl h-[80vh]"
          >
                  ...
                  <div className="p-6 space-y-6 overflow-y-auto h-full">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {([
                          { id: 'default', label: '默认', icon: Sun, color: 'from-slate-100 to-white dark:from-slate-700 dark:to-slate-800' },
                          { id: 'eye-care', label: '护眼', icon: Eye, color: 'from-emerald-50 to-emerald-100 dark:from-emerald-900/30 dark:to-emerald-800/30' },
                          { id: 'dark', label: '深色', icon: Moon, color: 'from-slate-800 to-slate-900 dark:from-slate-900 dark:to-slate-950' },
                        ] as const).map((mode) => (
                          <motion.button
                            key={mode.id}
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <Scroll size={16} className="text-slate-500 dark:text-slate-400" />
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          分页方式
                        </span>
                      </div>
                      <div className="flex p-1 bg-slate-100 dark:bg-slate-700 rounded-xl">
                        {([
                          ...
                          { id: 'pagination', label: '翻页', icon: FileText },
                        ] as const).map((mode) => (
                          <motion.button
                            key={mode.id}
                    ...
                    <div className="pt-2">
                      ...
                      >
                        <RefreshCcw size={16} />
                        <span className="text-sm font-medium">重置默认设置</span>
                      </motion.button>
                    </div>
                  </div>
                ) : (
