...
export const LevelTestNotificationBridge: React.FC<{
  onClose: () => void;
  onStart: () => void;
}> = ({ onClose, onStart }) => {
  const notice = useLevelTestNotificationStore((s) => s.notice);
  ...
  useEffect(() => {
    if (notice) {
      useNotificationManager.getState().show({
        ...
        dismissible: true,
        data: notice,
        action: notice.status === 'success' ? onStart : undefined,
        onDismiss: onClose,
      });
    } else {
      useNotificationManager.getState().dismiss('level-test');
    }
  }, [notice, onClose, onStart]);
