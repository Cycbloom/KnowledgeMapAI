      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-gray-500 dark:text-gray-400">学习进度</span>
        ...
        {pathDetail.target_completion_date && (
          <div className="mt-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span>目标完成日期：{new Date(pathDetail.target_completion_date).toLocaleDateString('zh-CN')}</span>
          </div>
        )}
      </div>
        ...
        <div className="space-y-1 px-2">
          {nodes.length === 0 ? (
            ...
            nodes.map((node: any, index: number) => {
              ...
              const isCurrentNode = node.knowledge_point_id === currentNodeId || node.id === currentNodeId;
              const estimatedTime = node.estimated_time || node.estimated_minutes;
                  ...
                  <div className="flex-shrink-0 flex flex-col items-center">
                    ...
                    {index < nodes.length - 1 && (
                      <div
                        className={`
                          w-0.5 h-6 mt-1
                          ${nodes[index + 1]?.status === 'completed' || status === 'completed'
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="p-2 bg-gray-50 dark:bg-slate-800 rounded-lg">
            ...
            <div className="text-[10px] text-gray-500 dark:text-gray-400">已完成</div>
          </div>
          <div className="p-2 bg-gray-50 dark:bg-slate-800 rounded-lg">
          ...
          <div className="p-2 bg-gray-50 dark:bg-slate-800 rounded-lg">
            <div className="text-lg font-bold text-gray-500">
              {nodes.filter((n: any) => n.status === 'pending' || !n.status).length}
            </div>
            <div className="text-[10px] text-gray-500 dark:text-gray-400">待学习</div>
          </div>
        </div>
      </div>
    </div>
