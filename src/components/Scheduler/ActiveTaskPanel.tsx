...
export const ActiveTaskPanel: React.FC<ActiveTaskPanelProps> = ({
  ...
  const config = QUEUE_CONFIG[task.queue_level as keyof typeof QUEUE_CONFIG] || QUEUE_CONFIG[2];
  const IconComponent = config.icon;
  
  const { timeLeft, isActive, startTimer, pauseTimer, setTaskId, setDuration } = useFocusStore();
  ...
  const progress = timeSlice > 0 ? ((timeSlice * 60 - timeLeft) / (timeSlice * 60)) * 100 : 0;
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        `}
      >
        <div className={`absolute top-0 left-0 h-1 bg-gradient-to-r ${config.gradient}`} style={{ width: `${Math.min(progress, 100)}%` }} />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${config.accentColor} bg-white/50 dark:bg-slate-800/50`}>
                  Q{task.queue_level}
                </span>
              </div>
              {task.description && (
          ...
          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-2xl font-mono font-bold ${config.accentColor}`}>
                  {formatTime(timeLeft)}
                </span>
              </div>
