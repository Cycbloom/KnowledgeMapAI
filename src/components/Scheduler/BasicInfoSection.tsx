...
export const BasicInfoSection: React.FC<BasicInfoSectionProps> = ({ task }) => {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-900 dark:text-white">{t('scheduler.taskWorkbench.basicInfo.title')}</h3>
      
      {task.description && (
        <div>
          <label className="text-sm text-gray-500 dark:text-gray-400">{t('scheduler.taskWorkbench.description')}</label>
      ...
      {task.context && (
        <div>
          <label className="text-sm text-gray-500 dark:text-gray-400">{t('scheduler.taskWorkbench.basicInfo.context')}</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {task.deadline && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <label className="text-xs text-gray-500 dark:text-gray-400">{t('scheduler.taskWorkbench.deadline')}</label>
              <p className="text-sm text-gray-700 dark:text-gray-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <label className="text-xs text-gray-500 dark:text-gray-400">{t('scheduler.taskWorkbench.basicInfo.priority')}</label>
            <p className="text-sm text-gray-700 dark:text-gray-300">
              P{task.priority}
            </p>
          </div>
      ...
      {task.task_type === 'long_term' && (
        <div>
          <label className="text-sm text-gray-500 dark:text-gray-400">{t('scheduler.taskWorkbench.basicInfo.completionProgress')}</label>
          <div className="mt-2">
            <div className="flex justify-between text-sm mb-1">
              <span>{task.progress_percentage}%</span>
              {task.required_time_slots && (
                <span className="text-gray-400">{t('scheduler.taskWorkbench.basicInfo.estimatedTimeSlicesHint', { count: task.required_time_slots })}</span>
              )}
            </div>
