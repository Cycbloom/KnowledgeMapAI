...
const QUEUE_COLORS = {
  0: { bg: 'bg-cyan-500/20', text: 'text-cyan-400', border: 'border-cyan-500/30' },
...
export const ListView: React.FC<ListViewProps> = ({
  ...
  const formatDeadline = (date?: string) => {
    if (!date) return { text: '--', color: 'text-slate-500' };
    const d = new Date(date);
    const now = new Date();
    ...
    return { text: d.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' }), color: 'text-slate-400' };
  };
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4 gap-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 rounded-lg bg-slate-800/50 border border-slate-700/50 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 w-64"
            />
          </div>
        ...
        <div className="text-sm text-slate-400">
          共 <span className="text-white font-medium">{filteredAndSortedTasks.length}</span> 个任务
        </div>
      </div>
        ...
        {showFilters && (
          ...
          >
            <div className="flex items-center gap-4 p-3 rounded-lg bg-slate-800/30 border border-slate-700/30">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500">队列:</span>
                <div className="flex gap-1">
                  {[0, 1, 2].map((level) => (
                    <button
                      key={level}
      ...
      <div className="flex-1 overflow-hidden rounded-xl border border-slate-700/50 bg-slate-900/60 backdrop-blur-sm">
                ...
                {COLUMNS.map((column) => (
                  <th
                    ...
                    className={`
                      ...
                      ${column.id !== 'actions' && column.id !== 'tags' ? 'cursor-pointer hover:text-slate-200' : ''}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            {task.priority >= 3 && <span className="text-red-400">★</span>}
                            <span className="text-slate-300">{task.priority || 0}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {task.tags?.slice(0, 2).map((tag, i) => (
                              <span key={i} className="px-1.5 py-0.5 rounded text-xs bg-indigo-500/20 text-indigo-400">
                                {tag}
                              </span>
                            ))}
                          ...
                          <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                            ...
                            {onEditTask && (
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-slate-700 text-slate-400 hover:text-amber-400 transition-all"
                                title="编辑"
                              >
                                <Edit2 size={14} />
                              </button>
                            ...
                            {onDeleteTask && (
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-slate-700 text-slate-400 hover:text-red-400 transition-all"
                                title="删除"
                              >
                                <Trash2 size={14} />
                              </button>
