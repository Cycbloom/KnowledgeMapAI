...
export const CalendarPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (tasksRes.success) {
        const calendarEvents: CalendarEvent[] = (tasksRes.data || []).map((task: any) => ({
          id: task.id,
          title: task.title,
          description: task.description,
          start: task.scheduled_start || task.deadline || task.created_at,
          end: task.scheduled_end,
          type: task.tags?.includes('学习') ? 'study' : task.tags?.includes('复习') ? 'review' : 'task',
      ...
      if (executionsRes.success) {
        const executionEvents: ExecutionEvent[] = (executionsRes.data || []).map((exec: any) => ({
          id: exec.id,
  ...
  const handleCreateTask = async () => {
    ...
    try {
      await api.scheduler.createTask({
        ...
        tags: taskForm.tags,
        queue_level: taskForm.priority >= 3 ? 0 : taskForm.priority >= 2 ? 1 : 2,
      });
  ...
  const handleEventDrop = useCallback(async (dropInfo: EventDropInfo) => {
    try {
      ...
      addMessage({ type: 'error', content: error.message || '更新任务时间失败' });
    }
  }, [addMessage]);
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setTaskForm({ ...taskForm, deadline: new Date(e.target.value) })}
                    className={`w-full px-3 py-2 rounded-lg border ${
                      isDark
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setTaskForm({ ...taskForm, estimated_duration: parseInt(e.target.value) || 30 })}
                      className={`w-full px-3 py-2 rounded-lg border ${
                        isDark
                          ? 'bg-slate-700 border-slate-600 text-white'
                          : 'bg-gray-50 border-gray-200 text-gray-900'
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className={`p-4 rounded-lg ${isDark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>
                    💡 华为手机用户：打开"日历"应用 → 点击右上角菜单 → 选择"订阅日历" → 粘贴 WebCal 链接
                  </p>
                </div>
              </div>
            </motion.div>
