...
import { Plus, Pencil, Trash2, Search, X, BookOpen, FileText, Briefcase, PieChart, Sparkles } from 'lucide-react';
    ...
    <div className={`min-h-screen ${isDark ? 'bg-slate-900' : 'bg-gray-50'}`}>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ${
                      template.category === 'learning' ? 'bg-blue-50 text-blue-600' :
                      template.category === 'story' ? 'bg-purple-50 text-purple-600' :
                      template.category === 'project' ? 'bg-green-50 text-green-600' :
      ...
      {isCreating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className={`w-full max-w-md rounded-2xl shadow-2xl p-6 md:p-8 ${
            ...
            <form onSubmit={handleUpdateTemplate} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  value={newTemplateDescription}
                  onChange={(e) => setNewTemplateDescription(e.target.value)}
                  placeholder={editingTemplate.description || '简要描述该模板的用途...'}
                  className={`w-full px-4 py-3 rounded-xl border outline-none transition-all resize-none ${
                    isDark 
                      ? 'bg-slate-900 border-slate-700 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500' 
                      : 'bg-gray-50 border-gray-200 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500'
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button 
                  type="submit" 
                  className="flex-1 px-4 py-3 rounded-xl font-medium bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  disabled={updateTemplateMutation.isPending || !newTemplateName}
                >
