...
export const mobileAiApi = {
  ...
  batchGenerateCards: (node_ids: string[], config: { types?: string[]; count?: number; pack_template?: string; provider?: string; model?: string }) => {
    ...
    return mobileAiClient.post('/ai/batch-generate-cards', { node_ids, config: payloadConfig });
  },
  ...
  documentToGraph: async (data: { graph_id: string; file: File }) => {
    ...
    const fullUrl = baseURL ? `${baseURL}/ai/document-to-graph` : '/ai/document-to-graph';
    const response = await fetch(fullUrl, {
      method: 'POST',
      headers: {
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  imageToGraph: async (formData: FormData) => {
    const token = useStore.getState().token;
    const baseURL = getCloudApiBaseUrl() || '';
    const fullUrl = baseURL ? `${baseURL}/ai/image-to-graph` : '/ai/image-to-graph';
  ...
  aiActions: {
    list: (graphId?: string) => mobileAiClient.get(`/ai-actions${graphId ? `?graph_id=${graphId}` : ''}`),
    create: (data: Partial<AIAction>) => mobileAiClient.post('/ai-actions', data),
    update: (id: string, data: Partial<AIAction>) => mobileAiClient.put(`/ai-actions/${id}`, data),
    delete: (id: string) => mobileAiClient.delete(`/ai-actions/${id}`),
    execute: (data: { action_id: string; node_id: string; graph_id?: string }) => mobileAiClient.post('/ai-actions/execute', data),
  },
};
