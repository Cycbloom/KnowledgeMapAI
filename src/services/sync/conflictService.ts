...
class ConflictService {
  ...
  resolveConflict(conflict: SyncConflict, resolution: 'local' | 'remote' | 'merge'): SyncOperation {
    ...
    switch (resolution) {
      ...
      case 'remote':
        ...
        break;
      case 'merge':
        resolvedOperation = this.mergeOperations(conflict.localVersion, conflict.remoteVersion);
        break;
      default:
        resolvedOperation = conflict.remoteVersion;
    }
  ...
  // 合并操作
  private mergeOperations(localOp: SyncOperation, remoteOp: SyncOperation): SyncOperation {
    // 合并两个操作的记录
    const mergedRecord = {
      ...localOp.record,
      ...remoteOp.record,
