import { SyncOperation, SyncBatch, SyncDevice, SyncConflict } from './syncTypes';
import { deviceDiscoveryService } from './deviceDiscoveryService';
import { syncAuthService } from './syncAuthService';
import { conflictService } from './conflictService';
import { getOfflineQueue, addToOfflineQueue, clearOfflineQueue } from '../../utils/offlineStorage';
...
export class MobileSyncService {
  ...
  private async syncWithDevice(device: SyncDevice): Promise<void> {
    ...
    try {
      ...
      if (this.pendingOperations.length > 0) {
        ...
        const response = await fetch(`http://${device.ipAddress}:3001/api/sync/receive`, {
          method: 'POST',
          headers: {
        ...
        const response = await fetch(`http://${device.ipAddress}:3001/api/sync/send`, {
          method: 'GET',
          headers: {
            'X-Sync-Token': token,
            'X-Device-Id': this.deviceId
  ...
  private async processConflicts(): Promise<void> {
    ...
    const resolvedOperations = conflictService.autoResolveConflicts(this.conflicts);
