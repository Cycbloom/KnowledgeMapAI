-- Fix RPC functions after title column type change
-- Must drop and recreate functions when return type changes

DROP FUNCTION IF EXISTS get_user_graphs_with_counts(UUID);
DROP FUNCTION IF EXISTS get_user_trashed_graphs(UUID);
