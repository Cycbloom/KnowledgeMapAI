...
export class DashboardPage {
  ...
  constructor(page: Page) {
    ...
    this.deleteConfirmModal = page.locator('.fixed.inset-0.z-50:has-text("删除图谱")');
    this.deleteConfirmButton = page.locator('.fixed.inset-0.z-50:has-text("删除图谱") button:has-text("确定")');
    this.deleteCancelButton = page.locator('.fixed.inset-0.z-50:has-text("删除图谱") button:has-text("取消")');
    this.graphCardMenuButton = page.locator('[class*="group relative rounded-2xl"] button[title="删除图谱"]');
    this.noResultsState = page.locator('text=未找到相关图谱');
    this.firstGraphCard = page.locator('[class*="group relative rounded-2xl"]').first();
    this.favoriteFilterButton = page.locator('button:has-text("收藏")');
    this.sortDropdown = page.locator('button:has-text("排序"), select[data-testid="sort-select"]');
  }
  ...
  async getGraphCardByTitle(title: string) {
    return this.page.locator(`[class*="group relative rounded-2xl"]:has-text("${title}")`);
  }
  ...
  async toggleFavorite(title: string) {
    const card = this.getGraphCardByTitle(title);
    await card.hover();
    const favoriteButton = card.locator('button[title="收藏图谱"], button[title="取消收藏"]');
  ...
  // 分页相关方法
  async goToNextPage() {
    const nextButton = this.page.locator('button:has(svg[class*="chevron-right"]), button:has(svg.lucide-chevron-right)');
  ...
  async goToPreviousPage() {
    const prevButton = this.page.locator('button:has(svg[class*="chevron-left"]), button:has(svg.lucide-chevron-left)');
  ...
  async waitForGraphsToLoad(timeout = 10000) {
    await this.page.waitForSelector('[class*="group relative rounded-2xl"]', { 
      state: 'attached', 
      timeout 
    }).catch(() => {
      // 如果没有图谱卡片，检查空状态
  ...
  async selectSortOption(option: string) {
    const sortDropdown = this.page.locator('button:has-text("排序"), select[data-testid="sort-select"]');
    if (await sortDropdown.count() > 0) {
      const isSelect = await sortDropdown.evaluate(el => el.tagName === 'SELECT');
      if (isSelect) {
  ...
  async filterByFavorites() {
    const favoriteFilter = this.page.locator('button:has-text("收藏"), button[title="仅显示收藏"]');
  ...
  async clearFavoriteFilter() {
    const activeFilter = this.page.locator('button.bg-blue-600:has-text("收藏"), button[aria-pressed="true"]');
  ...
  async openShareMenu(title: string) {
    const card = this.getGraphCardByTitle(title);
    await card.hover();
    const shareButton = card.locator('button[title*="分享"], button:has(svg[class*="share"])');
  ...
  async isShareMenuVisible() {
    const shareMenu = this.page.locator('.fixed:has-text("分享"), [role="dialog"]:has-text("分享")');
  ...
  async closeShareMenu() {
    ...
    if (await shareMenu.count() > 0) {
      const closeButton = shareMenu.locator('button:has(svg[class*="x"]), button:has-text("关闭")').first();
  ...
  async getShareLink() {
    const shareInput = this.page.locator('input[value*="http"], input[readonly]');
  ...
  async copyShareLink() {
    const copyButton = this.page.locator('button:has-text("复制"), button:has-text("Copy")');
  ...
  async toggleGraphVisibility(title: string) {
    const card = this.getGraphCardByTitle(title);
    await card.hover();
    const visibilityButton = card.locator('button[title*="公开"], button[title*="私有"]');
